---
description: 'Дополнительные сведения: предупреждение pragma'
...
|*После*|Отображение указанных сообщений только один раз.|
...
Директива pragma `warning( push )` хранит текущее состояние предупреждения для каждого предупреждения. Директива pragma `warning( push, n )` хранит текущее состояние для каждого предупреждения и устанавливает для глобального уровня предупреждений значение *n*.

Директива pragma `warning( pop )` извлекает Последнее состояние предупреждения, отправленное в стек. Все изменения, внесенные в состояние предупреждения между *принудительной* отправкой и *точкой* подключения, отменяются. Рассмотрим следующий пример.
