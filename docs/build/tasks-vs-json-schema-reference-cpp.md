---
description: Дополнительные сведения. Справочник по схеме tasks.vs.json (C++)
...
|-|-|-|
|`taskLabel`|строка| (Обязательно). Задает метку задачи, используемую в пользовательском интерфейсе.|
...
|Свойство|Тип|Описание|
|-|-|-|
|`verbosity`|строка| Указывает выходные значения verbosityAllowed сборки проекта MSBuild: "Quiet", "Minimal", "Normal", "Detailed", "Diagnostic".|
