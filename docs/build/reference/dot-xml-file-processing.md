---
description: Дополнительные сведения:. Обработка XML-файлов
...
- Первая часть строки идентификатора определяет тип идентифицируемого члена. Это один символ, за которым следует двоеточие. Используются следующие типы элементов.
  ...
  | Знак | Описание |
  |---------------|-----------------|
  | Нет | namespace<br /><br /> Вы не можете добавить комментарии документации для пространства имен, но можете создать для него cref-ссылки. |
  ...
  | E | event |
  | ! | текст ошибки<br /><br /> Остальная часть строки предоставляет сведения об ошибке. Компилятор КОМПИЛЯТОРОМ MSVC создает сведения об ошибке для ссылок, которые не могут быть разрешены. |
...
- Для свойств и методов, имеющих аргументы, список этих аргументов заключается в круглые скобки и указывается в конце. Если аргументы не используются, скобки отсутствуют. Аргументы разделяются запятыми. Кодировка каждого аргумента указывается в точности так, как она закодирована в сигнатуре .NET Framework.
  ...
  - ELEMENT_TYPE_PINNED представляется как "^" после имени измененного типа. Компилятор КОМПИЛЯТОРОМ MSVC никогда не создает это.

  - ELEMENT_TYPE_CMOD_REQ представляется как "&#124;" с указанием полного имени класса модификатора после имени измененного типа. Компилятор КОМПИЛЯТОРОМ MSVC никогда не создает это.
  ...
  - ELEMENT_TYPE_GENERICARRAY представляется как "[?]" после типа элемента для массива. Компилятор КОМПИЛЯТОРОМ MSVC никогда не создает это.
  ...
  - ELEMENT_TYPE_FNPTR представляется как "=FUNC:`type`(*подпись*)", где `type` обозначает возвращаемый тип, а *подпись* представляет аргументы метода. Если аргументы не используются, скобки опускаются. Компилятор КОМПИЛЯТОРОМ MSVC никогда не создает это.
...
- Для универсальных типов имя типа завершается символом обратного апострофа и числом, которое обозначает количество параметров универсального типа.  например следующие.
