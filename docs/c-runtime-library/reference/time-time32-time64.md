...
**время** является оболочкой для **_time64** и **time_t** по умолчанию эквивалентно **__time64_t**. Если необходимо заставить компилятор интерпретировать **time_t** как старый 32-разрядный **time_t**, можно определить **_USE_32BIT_TIME_T**. Это не рекомендуется, так как в приложении может произойти сбой после 18-го января 2038 года; использование этого макроса не поддерживается на 64-разрядных платформах.
