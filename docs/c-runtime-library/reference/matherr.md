---
description: 'Дополнительные сведения: _matherr'
title: _matherr
ms.date: 04/05/2018
api_name:
- _matherr
...
Для специальной обработки ошибок можно указать другое определение **_matherr**. При использовании динамически связанной версии библиотеки времени выполнения C (CRT) можно заменить подпрограмму **_matherr** по умолчанию в клиентском исполняемом файле с определенной пользователем версией. Однако нельзя заменить подпрограммы по умолчанию **_matherr** в клиенте DLL библиотеки DLL CRT.

При возникновении ошибки в математических подпрограммых **_matherr** вызывается с указателем на **_exceptionную** структуру типа (определенную в \<math.h> ) в качестве аргумента. Структура **_exception** содержит следующие элементы.
