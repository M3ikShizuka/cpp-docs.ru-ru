---
description: 'Дополнительные сведения: _calloc_dbg'
title: _calloc_dbg
ms.date: 11/04/2016
api_name:
- _calloc_dbg
...
Параметры *filename* и *LineNumber* доступны только при явном вызове **_calloc_dbg** или при определении константы препроцессора [_CRTDBG_MAP_ALLOC](../../c-runtime-library/crtdbg-map-alloc.md) .
...
**_calloc_dbg** — это отладочная версия функции [calloc](calloc.md) . Если [_DEBUG](../../c-runtime-library/debug.md) не определен, каждый вызов **_calloc_dbg** сокращается до вызова **calloc**. Как **calloc** , так и **_calloc_dbg** *выделяют блоки* памяти в базовой куче, но **_calloc_dbg** предлагает несколько функций отладки:
...
- *имя файла* / параметр *LineNumber* для определения источника запросов на выделение.
