...
|--------------|---------------------|
|**`fopen_s`**|`<stdio.h>`|
|**`_wfopen_s`**|`<stdio.h>` либо `<wchar.h>`|
...
**`c`** Параметры, **`n`** и **`t`** *`mode`* являются расширениями Майкрософт для и не должны **`fopen_s`** [`_fdopen`](fdopen-wfdopen.md) использоваться там, где требуется переносимость ANSI.
