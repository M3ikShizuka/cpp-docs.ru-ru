---
description: 'Дополнительные сведения: сочетания клавиш (редактор изображений C++ для значков)'
...
|-------------|----------|-----------------|
|Image. Аирбруштул|**CTRL** + **A**|Рисование с помощью аэрографа с выбранным размером и цветом.|
|Image.BrushTool|**CTRL** + **B**|Рисование с помощью кисти с выбранной формой, размером и цветом.|
|Image. Копяндаутлинеселектион|**CTRL**  +  **SHIFT**  +  **U**|Копирование текущего выделенного фрагмента и формирование его контура. Если цвет фона содержится в текущем выделенном фрагменте, он будет исключен, если выбран [прозрачный](./image-editor-for-icons.md) .|
|Image.DrawOpaque|**CTRL** + **J**|Делает текущий выбор [непрозрачным или прозрачным](./image-editor-for-icons.md).|
...
|Image.NewImageType|**Вставить**|Открывает [ \<Device> диалоговое окно Создание типа образа](./creating-an-icon-or-other-image-image-editor-for-icons.md) , с помощью которого можно создать изображение для другого типа изображений.|
...
|Image.ShowGrid|**CTRL**  +  **ALT**  +  **С**|Переключает пиксельную сетку (устанавливает или снимает флажок " **Пиксельная сетка** " в [диалоговом окне "Параметры сетки](./image-editor-for-icons.md)").|
|Image.ShowTileGrid|**CTRL**  +  **SHIFT**  +  **ALT**  +  **С**|Переключает сетку плиток (выбирает или снимает параметр **сетки плитки** в [диалоговом окне "Параметры сетки](./image-editor-for-icons.md)").|
...
|Image.SmallerBrush|  +  CTRL **-** знак|Уменьшение размера кисти на один пиксель в каждом направлении. Чтобы увеличить размер кисти, см. Image.LargerBrush в этой таблице.|
|Image.TextTool|**CTRL** + **T**|Открывает [диалоговое окно «текст»](./image-editor-for-icons.md).|
