---
description: 'Дополнительные сведения: Редактор сведений о версии (C++)'
...
- **Редактор сведений о версии** задает свойство **Debug** с помощью `#ifdef` в скрипте ресурса на основе `_DEBUG` флага сборки.

- Если `Private Build` ключ содержит **значение** , заданное в таблице **сведений о версии** , то соответствующее свойство **сборки** в окне **свойств** для `FILEFLAGS` ключа будет иметь значение **true**. Если **значение** пустое, свойство будет иметь **значение false**. Аналогичным образом, **Специальный ключ сборки** в таблице **сведений о версии** привязан к **Специальному** свойству сборки для `FILEFLAGS` ключа.
