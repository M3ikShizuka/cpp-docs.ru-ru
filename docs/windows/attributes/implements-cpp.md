...
Указывает интерфейсы диспетчеризации, которые должны быть членами coclass-класса IDL.
...
*interfaces*;<br/>
Разделенный запятыми список интерфейсов, которые будут членами кокласс IDL. Для указания одного интерфейса реализуется сокращенный метод **(** *interface_name* **)**.
...
Предположим, что приведен следующий IDL-файл, который будет доступен компилятору.
