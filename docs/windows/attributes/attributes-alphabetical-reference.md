---
description: 'Дополнительные сведения: Алфавитный справочник по атрибутам'
...
|[обладателем](licensed.md)|Указывает, что компонент, к которому применяется, имеет лицензию, и его необходимо создать с помощью `IClassFactory2` .|
