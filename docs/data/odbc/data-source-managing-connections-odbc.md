---
description: 'Дополнительные сведения о: источник данных: Управление соединениями (ODBC)'
...
1. Создайте `CDatabase` объект.

1. Вызовите `OpenEx` его `Open` функцию члена или.
...
Перед вызовом функции члена класса необходимо закрыть все открытые наборы записей `Close` `CDatabase` . В наборах записей, связанных с `CDatabase` объектом, который необходимо закрыть, любые отложенные `AddNew` `Edit` операторы или отменяются и выполняется откат всех ожидающих транзакций.

#### <a name="to-disconnect-from-a-data-source"></a>Отключение от источника данных

1. Вызовите `CDatabase` функцию [закрытия](../../mfc/reference/cdatabase-class.md#close) элемента объекта.
...
1. Вместо уничтожения объекта вызовите его `OpenEx` `Open` функцию члена или.
