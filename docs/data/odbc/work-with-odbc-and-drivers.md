---
description: 'Дополнительные сведения: работа с ODBC и драйверами'
title: Работа с ODBC и драйверами
ms.date: 11/04/2016
helpviewer_keywords:
- ODBC drivers, using
ms.assetid: ae1bf357-4ca4-4748-9336-8fc317f294ab
ms.openlocfilehash: 6197805a2906480684cb866de2a451de70fdec9f
ms.sourcegitcommit: d6af41e42699628c3e2e6063ec7b03931a49a098
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 12/11/2020
ms.locfileid: "97246153"
---
# <a name="work-with-odbc-and-drivers"></a>Работа с ODBC и драйверами

ODBC позволяет создавать приложения, не зависящие только от одной системы управления базами данных (СУБД). Можно написать одно приложение, которое может обрабатывать несколько СУБД, для которых пользователи имеют правильный драйвер ODBC для конкретной СУБД.

## <a name="in-this-section"></a>в этом разделе

- [Определение схемы источника данных ODBC во время выполнения](../../data/odbc/data-source-determining-the-schema-of-the-data-source-odbc.md)

- [Получение описания драйвера с помощью администратора ODBC](../../data/odbc/data-source-programmatically-configuring-an-odbc-data-source.md)

- [Получение справки в Интернете по драйверу ODBC](../../data/odbc/data-source-programmatically-configuring-an-odbc-data-source.md)

- [Программное создание таблицы в источнике данных ODBC](../../data/odbc/data-source-programmatically-creating-a-table-in-an-odbc-data-source.md)

- [Вызов функций API ODBC непосредственно в MFC](../../data/odbc/odbc-calling-odbc-api-functions-directly.md)

## <a name="see-also"></a>См. также раздел

[Открытие подключения к базе данных (ODBC)](../../data/odbc/open-database-connectivity-odbc.md)
