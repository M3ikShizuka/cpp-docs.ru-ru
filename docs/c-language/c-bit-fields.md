---
description: 'Подробнее о следующем: Битовые поля в C'
...
Битовые поля, определенные как **`int`** , обрабатываются как **`signed`** . Расширение Microsoft к стандарту ANSI C допускает битовые поля типов **`char`** и **`long`** (как **`signed`** , так и **`unsigned`** ). Неименованные битовые поля с базовым типом **`long`** , **`short`** или **`char`** ( **`signed`** или **`unsigned`** ) принудительно устанавливают выравнивание по границе, соответствующей этому типу.
