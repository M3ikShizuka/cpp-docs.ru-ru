...
Максимальный размер массива определяется параметром **size_t**. Определенный в файле заголовка STDDEF.H параметр **size_t** представляет собой значение типа **`unsigned int`** с диапазоном от 0x00000000 до 0x7CFFFFFF.
