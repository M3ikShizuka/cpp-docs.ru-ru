...
*cv_qualifier*<br/>
**`const`** или **`volatile`** квалификаторы. По умолчанию закрепляющий указатель имеет значение **`volatile`** . Он является избыточным, но не является ошибкой для объявления указателя закрепления **`volatile`** .
