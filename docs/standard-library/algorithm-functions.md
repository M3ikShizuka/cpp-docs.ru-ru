---
description: Дополнительные сведения о &lt; &gt; функциях алгоритма
...
## <a name="equal"></a><a name="equal"></a> равномерно
...
## <a name="fill"></a><a name="fill"></a> заполнения
...
## <a name="generate"></a><a name="generate"></a> привести
...
## <a name="includes"></a><a name="includes"></a> том
...
Вторая функция шаблона ведет себя так же, как первая, за исключением того, что в  `std::less<>` качестве условия упорядочивания кучи вместо него используется предикат «пред».
...
При использовании алгоритма `min` очень редко объекты передаются как параметры. Большинство алгоритмов библиотеки стандартных программ C++ работают в диапазоне элементов, позиция которых задается итераторами, передаваемыми в качестве параметров. Если требуется функция, которая работает в диапазоне элементов, используйте [min_element](algorithm-functions.md#min_element). [](../cpp/constexpr-cpp.md) в `initializer_list` Visual Studio 2017 для перегрузок включена constexpr.
...
## <a name="mismatch"></a><a name="mismatch"></a> несовместим
...
## <a name="partition"></a><a name="partition"></a> диска
...
Сложность линейная: существуют приложения, которые являются допустимыми и не поддерживают `(last - first)`  `(last - first)/2` замену.
...
## <a name="replace"></a><a name="replace"></a> восстановить
...
## <a name="rotate"></a><a name="rotate"></a> повернуть
...
## <a name="search"></a><a name="search"></a> осуществлять
...
## <a name="shuffle"></a><a name="shuffle"></a> носить
...
## <a name="transform"></a><a name="transform"></a> Преобразует
