---
description: 'Дополнительные сведения: &lt; Служебные &gt; функции'
...
Для перегрузок, у которых нет параметра index, возвращаемый элемент выводится аргументом типа. Вызов `get<T>(Tuple)` выдаст ошибку компилятора, если *PR* содержит больше или меньше одного элемента типа T.
...
- В противном случае возвращаемый тип `T` имеет значение `decay<T>::type`. Если [класс Decay](../standard-library/decay-class.md) не поддерживается, возвращаемый тип совпадает с `T` типом входных данных `T` .

Возвращаемый тип `U` аналогичным образом определяется по типу ввода `U`.

Одно из преимуществ `make_pair` заключается в том, что типы сохраняемых объектов определяются компилятором автоматически, и их не нужно указывать явно. Не используйте явные аргументы шаблона, например `make_pair<int, int>(1, 2)` при использовании, `make_pair` так как они подробно изменяют и добавляют сложные ссылки rvalue, которые могут вызвать сбой компиляции. В данном случае правильный синтаксис — `make_pair(1, 2)`
...
*АРГ*\
Аргумент для приведения. Несмотря на то, что тип *аргумента* по определен как ссылка rvalue, `move` также принимает аргументы lvalue, так как ссылки lvalue могут быть привязаны к ссылкам rvalue.
...
Одно из преимуществ `swap` заключается в том, что типы сохраняемых объектов определяются компилятором автоматически, и их не нужно указывать явно. Не используйте явные аргументы шаблона, например `swap<int, int>(1, 2)` при использовании, `swap` так как они подробно изменяют и добавляют сложные ссылки rvalue, которые могут вызвать сбой компиляции.
...
Преобразует значение в символьную строку, заполняя диапазон `[first, last)` , где должно `[first, last)` быть допустимым диапазоном.
