---
description: 'Дополнительные сведения о: gslice Class'
title: Класс gslice
ms.date: 11/04/2016
f1_keywords:
- valarray/std::gslice
...
Класс хранит параметры, характеризующие объект типа [gslice_array](../standard-library/gslice-array-class.md). Подмножество valarray создается косвенно, когда объект класса gslice отображается как аргумент для объекта класса [valarray](../standard-library/valarray-class.md#op_at) **\<Type>** . Хранимые значения, задающие подмножество, выбираемое из родительского valarray, включают:

- Начальный индекс.

- Вектор длины класса `valarray<size_t>` .

- Вектор STRIDE класса `valarray<size_t>` .
...
|[шага](#stride)|Находит расстояние между элементами в общем срезе `valarray`.|
