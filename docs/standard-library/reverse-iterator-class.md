---
description: 'Дополнительные сведения о: reverse_iterator классе'
...
Объект `reverse_iterator` , для которого требуется двунаправленный итератор, не должен вызывать функции-члены `operator+=` ,,, `operator+` `operator-=` `operator-` или `operator[]` , которые могут использоваться только с итераторами произвольного доступа.

Диапазон итератора — [*первый*, *последний*), где квадратная скобка слева указывает на включение *первого* , а круглые скобки справа указывают на включение элементов вплоть до, но за исключением самого *последнего* . Одни и те же элементы включены в обратную последовательность [ **Редакция**  -  *First*, **Редакция**  -  *Last*), так что если *последним* является однонаправленный элемент в последовательности, первый элемент, номер которого   -  *первым* в инвертированной последовательности указывает на \* (*Last* -1). Идентификатор, который связывает все обратные итераторы с их базовыми итераторами:
...
|[вид](#pointer)|Тип, содержащий указатель на элемент, к которому обращается итератор `reverse_iterator`.|
...
|[operator + +](#op_add_add)|Увеличение `reverse_iterator` до следующего элемента.|
|[operator + =](#op_add_eq)|Добавление заданного смещения из итератора `reverse_iterator`.|
|[станции](#operator-)|Вычитание смещения из итератора `reverse_iterator` с возвратом итератора `reverse_iterator`, который обращается к элементу в позиции со сдвигом.|
...
## <a name="reverse_iteratorbase"></a><a name="base"></a> reverse_iterator:: базовый
...
Тип является синонимом типа TypeName для признака итератора `iterator_traits` \< **RandomIterator**> **::p оинтер**.
...
Оператор вычисляет текущее значение   +  *Off* и возвращает **\* this**.
...
Оператор возвращает значение <strong>\*</strong> ( **\* this**  +  `Off` ).
...
## <a name="reverse_iteratorpointer"></a><a name="pointer"></a> reverse_iterator::p оинтер
...
Тип является синонимом для признака модификатора TypeName `iterator_traits` \< *RandomIterator*> **:: Reference**.

### <a name="example"></a>Пример

Примеры объявления и использования см. в разделе [reverse_iterator:: operator&#91;&#93;](#op_at) или [reverse_iterator:: operator *](#op_star) `reference` .
