---
description: 'Дополнительные сведения: контейнеры стандартной библиотеки C++'
...
`array`Контейнер имеет некоторые сильные стороны `vector` , но его длина не так гибкая. Дополнительные сведения см. в разделе [Класс array](../standard-library/array-class-stl.md).

Контейнер `deque` (двусторонняя очередь) обеспечивает быструю вставку и удаление в начале и в конце контейнера. Он предоставляет преимущества для произвольного доступа и гибкой длины `vector` , но не непрерывно. Дополнительные сведения см. в разделе [Класс deque](../standard-library/deque-class.md).

`list`Контейнер — это двунаправленный связанный список, который обеспечивает двунаправленный доступ, быструю вставку и быстрое удаление в любом месте в контейнере, но не может получить случайный доступ к элементу в контейнере. Дополнительные сведения см. в разделе [Класс list](../standard-library/list-class.md).
...
Упорядоченные ассоциативные контейнеры (Map, multimap, Set и мультинаборы) теперь поддерживают разнородный Уточняющий запрос, что означает, что больше не требуется передавать точно тот же тип объекта, что и ключ или элемент в функциях-членах, таких как `find()` и `lower_bound()` . Вы можете передать объект любого типа, для которого определен перегруженный `operator<`, позволяющий выполнять сравнение с типом ключа.
...
В следующем примере показано, как перегрузить, `operator<` чтобы позволить пользователям выполнять `std::set` уточняющие запросы, просто передав небольшую строку, которую можно сравнить с элементом каждого объекта `BigObject::id` .
...
Так как адаптеры контейнеров не поддерживают итераторы, их нельзя использовать с алгоритмами стандартной библиотеки C++. Дополнительные сведения см. в разделе [Алгоритмы](../standard-library/algorithms.md).
