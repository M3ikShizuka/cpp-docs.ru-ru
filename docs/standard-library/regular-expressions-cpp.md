---
description: 'Дополнительные сведения: регулярные выражения (C++)'
...
ms.locfileid: "97159756"
---
# <a name="regular-expressions-c"></a>Регулярные выражения (C++)

Стандартная библиотека C++ поддерживает несколько грамматик регулярных выражений. В этом разделе обсуждаются варианты грамматики, доступные при использовании регулярных выражений.

## <a name="regular-expression-grammar"></a><a name="regexgrammar"></a> Грамматика регулярных выражений

Используемая грамматика регулярного выражения задается с помощью одного из `std::regex_constants::syntax_option_type` значений перечисления. Эти грамматики регулярных выражений определяются в std:: regex_constants:

- `ECMAScript`: Это ближайший к грамматике, используемой в языках JavaScript и .NET.
- `basic`: Основные регулярные выражения POSIX или Бре.
- `extended`: Расширенные регулярные выражения POSIX или ERE.
- `awk`: Это `extended` , но у него есть дополнительные escape-последовательности для непечатаемых символов.
- `grep`: Это `basic` , но также позволяет символам новой строки ("\n") разделять чередования.
- `egrep`: Это `extended` , но также позволяет символам новой строки разделять чередование.

По умолчанию, если грамматика не указана, `ECMAScript` предполагается. Можно указать только одну грамматику.

В дополнение к грамматике можно применить несколько флагов:

- `icase`: Не учитывать регистр при сопоставлении.
- `nosubs`: Игнорировать помеченные совпадения (то есть выражения в круглых скобках); подстановки не сохраняются.
- `optimize`: Сделайте сопоставление быстрее, за счет возможного увеличения времени создания.
- `collate`: Используйте последовательности параметров сортировки с учетом языкового стандарта (например, диапазоны в формате "[a-z]").

Для указания поведения обработчика регулярных выражений можно использовать ноль или более флагов вместе с грамматикой. Если указаны только флаги, `ECMAScript` предполагается использование грамматики.
...
- Escape-последовательность *идентификации* в форме " \\ `k` ", которая соответствует символу `k` в целевой последовательности.
...
В `ECMAScript` , `basic` и `grep` элемент может также быть *обратной ссылкой* на форму " \\ `dd` ", где `dd` представляет десятичное значение N, которое соответствует последовательности символов в целевой последовательности, совпадающей с последовательностью символов в n-й *группе записи*. Например, "(a)\1" соответствует целевой последовательности "aa", поскольку первая (и единственная) группа захвата соответствует первоначальной последовательности "a", а затем \1 соответствует завершающей последовательности "a".

В `ECMAScript` элемент также может являться одним из приведенных ниже объектов:

- *Группа, не* относящаяся к записи, в форме "(?: *часть выражения* )". Соответствует последовательности символов в целевой последовательности, которой соответствует шаблон между разделителями.
...
- *Escape-последовательность формата файла в формате* " \\ \\ ", "\a", "\b", "\f", "\n", "\r", "\t" или "\v". В целевой последовательности они соответствуют символам обратной косой черты, оповещения, стирания назад, перевода строки, возврата каретки, горизонтальной табуляции и вертикальной табуляции, в порядке перечисления.

- *Восьмеричная escape-последовательность* вида " \\ `ooo` ". Соответствует символу в целевой последовательности, представлением которого является значение, представленное одной, двумя или тремя восьмеричными цифрами `ooo`.
...
- "a?" соответствует целевой последовательности "" и целевой последовательности "a", но не целевой последовательности "AA".

- "a+" соответствует целевой последовательности "a", целевой последовательности "aa" и т.д., но не целевой последовательности "".

В `ECMAScript` все формы счетчика повторений могут сопровождаться символом "?", который обозначает *нежадное повторение*.

### <a name="concatenation"></a>Объединение

Элементы регулярного выражения с *числом повторений* или без него можно объединять для формирования более длинных регулярных выражений. Результирующее выражение соответствует целевой последовательности, которая является объединением последовательностей, соотнесенных по отдельным элементам. Например, "a {2,3} b" соответствует целевой последовательности "AAB" и целевой последовательности "aaab", но не соответствует целевой последовательности "AB" или целевой последовательности "ааааб".
...
|`extended`, `egrep`|{"(", ")", "{", ".", "[", " \\ ", " \* ", "^", "$", "+", "?", "&#124;"}|
|`awk`|`extended` плюс { '"', '/' }|
|`ECMAScript`|Все символы, кроме символов, которые могут быть частью идентификатора. Обычно сюда входят буквы, цифры, знаки "$", " \_ " и escape-последовательности Юникода. Дополнительные сведения см в спецификации языка ECMAScript.|
...
Группа незахвата помечает свое содержимое в качестве единого элемента в грамматике регулярного выражения, но не снабжает меткой целевой текст. Например, "(a) (?: b) \* (c)" соответствует целевому тексту "АББК" и связывает группу захвата 1 с частью последовательности "a" и группой захвата 2 с частью последовательности "c".

### <a name="non-greedy-repetition"></a>"Нежадное" повторение

"Нежадное" повторение использует наиболее краткую часть целевой последовательности, соответствующую шаблону. "Жадное" повторение использует наиболее длинное. Например, "(a +) (a \* b)" соответствует целевой последовательности "aaab". Если используется "нежадное" повторение, оно связывает группу захвата 1 с частью целевой последовательности "a" в начале данной последовательности, а группу захвата 2 — с частью целевой последовательности "aab" в конце данной последовательности. Если используется "нежадное" повторение, оно связывает группу захвата 1 с частью последовательности "aaa", а группу захвата 2 — с частью последовательности "b".
...
- " \* " имеет особое значение во всех случаях, за исключением случаев, когда это первый символ в регулярном выражении или первый символ, следующий за начальным "^" в регулярном выражении, или когда это первый символ в группе захвата или первый символ, следующий за начальным "^" в группе захвата.
...
- "(= AA) (a \* )" соответствует целевой последовательности "AAAA" и связывает группу захвата 1 с частью последовательности "AAAA".

- "(AA) (a \* )" соответствует целевой последовательности "AAAA" и связывает группу захвата 1 с частью последовательности "AA" в начале целевой последовательности и группой захвата 2 с частью последовательности "AA" в конце целевой последовательности.
...
|"$$"||"$"|
||" \\&"|"&"|
|"$ \` " (знак доллара, после которого стоит Обратная кавычка) | | Последовательность символов, предшествующая подпоследовательности, которая соответствует регулярному выражению ( `[match.prefix().first, match.prefix().second)` )|
...
|"$n"|"\n"|Последовательность символов, которая соответствует группе захвата в позиции `n` , где `n` — число от 0 до 9 ( `[match[n].first, match[n].second)` )|
||"\\\n"|"\n"|
|"$nn"||Последовательность символов, которая соответствует группе захвата в позиции `nn` , где `nn` — число от 10 до 99 ( `[match[nn].first, match[nn].second)` )|
