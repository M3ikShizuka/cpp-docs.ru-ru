---
description: 'Дополнительные сведения о: auto_ptr классе'
...
Оператор выбора возвращает [Get](#get) `( )` , поэтому элемент *AP* в выражении ->   ведет себя так же, как ( *AP*). **Get**()) — > **элемент**, где *AP* — это объект класса `auto_ptr` \< **Type**> . Следовательно, сохраненный указатель не должен иметь значение NULL и `Type` должен быть классом, структурой или типом объединения с `member` элементом.
