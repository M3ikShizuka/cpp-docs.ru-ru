---
description: Дополнительные сведения о классе List
title: Класс list
ms.date: 11/04/2016
f1_keywords:
- list/std::list
...
|[iterator](#iterator)|Тип, предоставляющий двунаправленный итератор, который может читать или изменять любой элемент в списке.|
...
|[пусто](#clear)|Удаляет все элементы списка.|
...
|[end](#end)|Возвращает итератор, адресующий расположение после последнего элемента в списке.|
...
Третья пара функций-членов вставляет диапазон, обозначенный [ `First` , `Last` ), перед элементом в конечном списке, на который ссылается,  и удаляет этот диапазон элементов из исходного списка. (Если `&Source == this` значение, диапазон `[First, Last)` не должен включать элемент, на который указывает, *где*.)
...
Вторая функция члена удаляет каждый элемент, который *удовлетворяет функции* предиката, когда он сравнивается с предыдущим элементом. Можно использовать любой объект бинарной функции, объявленный в \<functional> заголовке аргумента  , или можно создать собственный.
