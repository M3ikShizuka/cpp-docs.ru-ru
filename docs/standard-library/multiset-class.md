...
| [`contains`](#contains)<sup>C++ 20</sup> | Проверьте, существует ли элемент с указанным ключом в `multiset` . |
...
См. пример для [`get_allocator`](#get_allocator) для примера с помощью `allocator_type`
...
Двунаправленный итератор, обращающийся к первому элементу в `multiset` или в месте, где был выполнен пустой мультинабор.
...
Тип, предоставляющий двунаправленный итератор, который может читать **`const`** элемент в `multiset` .
...
Пример использования см. в примере для [`begin`](#begin) `const_iterator` .

## <a name="multisetconst_pointer"></a><a name="const_pointer"></a> `multiset::const_pointer`

Тип, предоставляющий указатель на **`const`** элемент в `multiset` .
...
В большинстве случаев для доступа к элементам объекта используется [итератор](#iterator) `multiset` .

## <a name="multisetconst_reference"></a><a name="const_reference"></a> `multiset::const_reference`

Тип, предоставляющий ссылку на **`const`** элемент, хранящийся в, `multiset` для чтения и выполнения **`const`** операций.
...
Тип, предоставляющий двунаправленный итератор, который может читать любой **`const`** элемент в `multiset` .
...
Тип `const_reverse_iterator` не может изменять значение элемента и используется для прохода по `multiset` в обратную.

### <a name="example"></a>Пример

См [`rend`](#rend) . Пример объявления и использования `const_reverse_iterator` .
...
*`key`*\
Ключ элементов, которые должны быть сопоставлены с `multiset` .

### <a name="return-value"></a>Возвращаемое значение

Число элементов в, `multiset` ключ сортировки которых соответствует ключу параметра.
...
В следующем примере демонстрируется использование `multiset` функции члена:: count.
...
`crbegin` используется с обратным мультинабором так же, как begin используется с `multiset` .

При возвращении значения `crbegin` `multiset` объект не может быть изменен.
...
`crend` используется с обратным мультинабором так же, как [`end`](#end) используется с `multiset` .

При возвращении значения `crend` `multiset` объект не может быть изменен.
...
*`args`*\
Аргументы, перенаправляемые для создания элемента, который будет вставлен в `multiset` .
...
**`true`** значение, если `multiset` пусто; значение **`false`** , если `multiset` не является пустым.
...
Итератор после конца. Если `multiset` пуст, то `multiset::end() == multiset::begin()`.

### <a name="remarks"></a>Remarks

**`end`** используется для проверки, прошел ли итератор конец своего мультинабора.
...
*`key`*\
Ключ аргумента, сравниваемый с ключом сортировки элемента из `multiset` искомого объекта.

### <a name="return-value"></a>Возвращаемое значение

Пара итераторов, первый из которых является [`lower_bound`](#lower_bound) ключом, а второй — [`upper_bound`](#upper_bound) ключом.

Для доступа к первому итератору пары `pr`, возвращаемой функцией-членом, нужно использовать `pr`. **`first`** и для разыменования итератора нижней границы используйте \* ( `pr` . **`first`**). Для доступа ко второму итератору пары `pr`, возвращаемой функцией-членом, нужно использовать `pr`. **`second`** и для разыменования итератора верхней границы используйте \* ( `pr` . **`second`**).
...
Для первых двух функций-членов — двунаправленный итератор, обозначающий первый элемент, оставшийся после удаленных элементов, или элемент, являющийся концом, `multiset` Если такого элемента не существует.

Для третьей функции члена возвращает количество элементов, удаленных из `multiset` .
...
Возвращает итератор, который ссылается на расположение элемента в с `multiset` ключом, эквивалентным указанному ключу.
...
*`key`*\
Значение ключа, которое должно быть сопоставлено с ключом сортировки элемента из `multiset` искомого объекта.

### <a name="return-value"></a>Возвращаемое значение

Итератор, который ссылается на расположение элемента с указанным ключом или расположение после последнего элемента в `multiset` ( `multiset::end()` ), если совпадение для ключа не найдено.

### <a name="remarks"></a>Remarks

Функция-член возвращает итератор, который ссылается на элемент, `multiset` ключ которого эквивалентен аргументу *`key`* в бинарном предикате, который вызывает упорядочивание на основе отношения сравнения "меньше".

Если возвращаемое значение `find` присваивается `const_iterator` , то `multiset` объект нельзя изменить. Если возвращаемое значение `find` присваивается `iterator` , `multiset` объект может быть изменен
...
Возвращает копию объекта распределителя, используемого для создания `multiset` .
...
*`Val`*\
Значение элемента, вставляемого в коллекцию `multiset` .

*`Where`*\
Место начала поиска правильной точки вставки. (Если эта точка непосредственно предшествует *`Where`* , то вставка может выполняться в периодической константе вместо логарифмического времени.)

*`ValTy`*\
Параметр шаблона, указывающий тип аргумента, который `multiset` может использоваться для создания элемента [`value_type`](../standard-library/map-class.md#value_type) , и идеального перенаправления в *`Val`* качестве аргумента.
...
*1*\
Аргумент функции-шаблона, соответствующий требованиям объекта [`input_iterator_tag`](../standard-library/input-iterator-tag-struct.md) , указывающего на элементы типа, который можно использовать для создания [1](../standard-library/map-class.md#value_type) объектов.
...
Функции-члены с одним элементом вставки (1) и (2) возвращают итератор в позицию, где новый элемент был вставлен в `multiset` .

Функции-члены с одним элементом с указанием (3) и (4) возвращают итератор, указывающий на позицию вставки нового элемента в `multiset` .
...
[`value_type`](../standard-library/map-class.md#value_type)Контейнером является typedef, принадлежащий контейнеру, а для набора `multiset<V>::value_type` — тип `const V` .

Функция-член Range (5) вставляет последовательность значений элементов в объект `multiset` , соответствующий каждому элементу, адресованному итератором в диапазоне `[First, Last)` , поэтому не `Last` вставляется. Контейнер функции-члена `end()` ссылается на позицию сразу после последнего элемента в контейнере. Например, оператор `s.insert(v.begin(), v.end());` пытается вставить все элементы `v` в `s`.

Функция-член списка инициализаторов (6) использует [`initializer_list`](../standard-library/initializer-list.md) для копирования элементов в `multiset` .

Для вставки элемента, созданного на месте (т. е. не выполняются операции копирования или перемещения), [`multiset::emplace`](#emplace) см [`multiset::emplace_hint`](#emplace_hint) . и.
...
Тип, предоставляющий постоянный [двунаправленный итератор](../standard-library/bidirectional-iterator-tag-struct.md) , который может считывать любой элемент в `multiset` .
...
Возвращает объект-функцию, `multiset` используемую для упорядочивания своих элементов, которая является параметром шаблона `Compare` .
...
[`key_compare`](#key_compare)И [`value_compare`](#value_compare) являются синонимами для параметра шаблона ``Compare`` . Оба типа предоставляются для классов Set и мультинаборов, где они идентичны, для совместимости с классами Map и multimap, где они отличаются.
...
Дополнительные сведения о см `Compare` . в подразделе "Примечания" статьи о [ `multiset` классе](../standard-library/multiset-class.md) .

### <a name="example"></a>Пример

[`key_comp`](#key_comp)Пример объявления и использования см. в примере для `key_compare` .

## <a name="multisetkey_type"></a><a name="key_type"></a> `multiset::key_type`

Тип, предоставляющий объект функции, который может сравнивать ключи сортировки для определения относительного порядка двух элементов в `multiset` .
...
Дополнительные сведения о см `Key` . в подразделе "Примечания" статьи о [ `multiset` классе](../standard-library/multiset-class.md) .

### <a name="example"></a>Пример

[`value_type`](#value_type)Пример объявления и использования см. в примере для ``key_type`` .
...
Объект `iterator` или `const_iterator` , который обращается к расположению элемента в с `multiset` ключом, который равен ключу аргумента или больше него, либо обращается к расположению, следующему за последним элементом в, `multiset` Если совпадение для ключа не найдено.
...
Максимально возможная длина `multiset` .
...
*`Right`*\
, для `multiset` которого сконструированный мультинабор является копией.
...
Все конструкторы хранят тип объекта распределителя, который управляет хранилищем памяти для `multiset` и затем может быть возвращен путем вызова [`get_allocator`](#get_allocator) . Параметр-распределитель часто не указывается в объявлениях класса и в макросах предварительной обработки, используемых для замены альтернативных распределителей.

Все конструкторы инициализируют свои мультинаборы.

Все конструкторы хранят объект функции типа Compare, который используется для установления порядка в ключах `multiset` и, которые впоследствии могут быть возвращены путем вызова метода [`key_comp`](#key_comp) .

Первые три конструктора указывают пустой начальный Мультинабор, второй задает тип функции сравнения ( *`Comp`* ), используемой при установлении порядка элементов, а третий явно указывает тип распределителя ( *`Al`* ) для использования. Ключевое слово **`explicit`** подавляет определенные виды автоматического преобразования типов.

Четвертый конструктор указывает копию объекта `multiset` *`Right`* .

Пятый конструктор указывает копию объекта `multiset` путем перемещения *`Right`* .

В шестом, седьмом и восьмом конструкторах указывается initializer_list, из которого копируются элементы.

Следующие три конструктора копируют диапазон `[First, Last)` `multiset` с путем увеличения явной точности при указании типа функции сравнения и распределителя.
...
`operator=` копирует или перемещает элементы в в *`Right`* это `multiset` , в зависимости от используемого ссылочного типа (lvalue или rvalue). Элементы, которые находились в этом `multiset` перед выполнением `operator=`, игнорируются.
...
Обратный двунаправленный итератор, обращающийся к первому элементу в обратную `multiset` или обращающийся к последнему элементу в необращенном порядке `multiset` .

### <a name="remarks"></a>Remarks

`rbegin` используется с обратным `multiset` так же, как rbegin используется с `multiset` .

Если возвращаемое значение `rbegin` присваивается `const_reverse_iterator` , то `multiset` объект нельзя изменить. Если возвращаемое значение `rbegin` присваивается `reverse_iterator` , то `multiset` объект можно изменить.
...
Обратный двунаправленный итератор, который обращается к расположению, следующему за последним элементом в обращении `multiset` (расположении, предшествующем первому элементу в неизмененном порядке `multiset` ).

### <a name="remarks"></a>Remarks

`rend` используется с обратным мультинабором так же, как [`end`](#end) используется с `multiset` .

Если возвращаемое значение `rend` присваивается `const_reverse_iterator` , то `multiset` объект нельзя изменить. Если возвращаемое значение `rend` присваивается `reverse_iterator` , то `multiset` объект можно изменить.
...
Тип `reverse_iterator` используется для прохода по `multiset` в обратную.

### <a name="example"></a>Пример

[`rbegin`](#rbegin)Пример объявления и использования см. в разделе пример `reverse_iterator` .
...
См. пример [`size`](#size) объявления и использования `size_type`
...
**Итератор** или `const_iterator` , который обращается к расположению элемента в `multiset` с ключом, который больше, чем ключ аргумента, или обращается к расположению, следующему за последним элементом в, `multiset` Если совпадение для ключа не найдено.
...
[`key_compare`](#key_compare)И [`value_compare`](#value_compare) являются синонимами для параметра шаблона `Compare` . Оба типа предоставляются для набора классов и `multiset` , где они идентичны, для совместимости с классами Map и `multimap` , где они отличаются.
...
Тип, предоставляющий объект функции, который может сравнивать два ключа сортировки для определения их относительного порядка в `multiset` .
...
[`key_compare`](#key_compare)И `value_compare` являются синонимами для параметра шаблона `Compare` . Оба типа предоставляются для набора классов и `multiset` , где они идентичны, для совместимости с классами Map и `multimap` , где они отличаются.

Более подробную информацию по `Compare` см. в разделе "Примечания" в документации к [классу multiset](../standard-library/multiset-class.md).

### <a name="example"></a>Пример

[`value_comp`](#value_comp)Пример объявления и использования см. в примере для `value_compare` .

## <a name="multisetvalue_type"></a><a name="value_type"></a> `multiset::value_type`

Тип, описывающий объект, хранящийся как элемент в качестве `multiset` его емкости в качестве значения.
...
[`key_type`](#key_type)И `value_type` являются синонимами для параметра шаблона `Key` . Оба типа предоставляются для классов Set и мультинаборов, где они идентичны, для совместимости с классами Map и multimap, где они отличаются.
