...
## <a name="using-_iterator_debug_level"></a>Использование _ITERATOR_DEBUG_LEVEL
...
Проверка итератора отладки также приводит к тому, что переменная-итератор, объявленная в **`for`** цикле, выходит из области действия, когда **`for`** заканчивается область цикла.
