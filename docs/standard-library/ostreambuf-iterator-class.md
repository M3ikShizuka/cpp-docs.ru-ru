---
description: 'Дополнительные сведения о: ostreambuf_iterator классе'
...
Функция-член возвращает значение **`true`** , если при любом предыдущем использовании члена `operator=` вызов функции регистр _ -> `sputc` вернул **EOF**.
