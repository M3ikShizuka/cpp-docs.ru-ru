...
> [!NOTE]
> В этой статье рассматривается тип языка C++ Standard в формате ISO **`enum`** и тип **класса Enum** с областью действия (или строго типизированный), который появился в C++ 11. Сведения об **открытых классах enum** или **закрытых типах классов enum** в c++/CLI и c++/CX см. в разделе [Класс Enum](../extensions/enum-class-cpp-component-extensions.md).
