---
description: 'Подробнее о: руководство по переносу: COM Spy'
...
ms.locfileid: "97322643"
---
# <a name="porting-guide-com-spy"></a>Руководство по переносу: COM Spy

Это вторая статья в серии статей, в которых описывается процесс обновления старых проектов Visual Studio C++ до последней версии среды Visual Studio. Пример кода в этом разделе был в последний раз скомпилирован с помощью Visual Studio 2005.
...
Эта команда регистрации после сборки нам больше не нужна. Вместо этого просто удалите команду настраиваемой сборки и укажите в параметрах **компоновщика** , чтобы зарегистрировать выходные данные.
...
Проблема заключается в том, что `i` объявляется как `UINT` и `lCount` объявляется как **`long`** , поэтому несовпадение со знаком и без знака. Было бы неудобно изменить тип `lCount` на `UINT` , так как он получает значение из `IMtsEventInfo::get_Count` , которое использует тип **`long`** и не находится в пользовательском коде. Поэтому мы добавим приведение в код. Приведение в стиле C выполняется для числового приведения, такого как this, но **`static_cast`** является рекомендуемым стилем.
...
Чтобы протестировать функциональные возможности приложения, мы создали **обслуживаемый компонент** с помощью шаблона проекта ATL, выбрали Добавление поддержки COM+ в мастер проектов ATL. Если вы еще не работали с обслуживаемыми компонентами, то не сможете создать его и получить регистрацию и доступность в системе или сети для использования другими приложениями. Приложение COM Spy предназначено для отслеживания активности обслуживаемых компонентов в качестве средства диагностики.
...
Затем мы создали и зарегистрировали его (вам потребуется запустить Visual Studio от имени администратора) и активировать его с помощью приложения **обслуживаемого компонента** на панели управления Windows. Мы создали проект Windows Forms в C#, перетащили кнопку в форму с панели элементов и дважды щелкнули ее для перехода к обработчику событий щелчка кнопкой мыши. Чтобы создать экземпляр компонента `Dog`, мы добавили следующий код.
