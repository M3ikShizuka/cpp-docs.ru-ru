---
description: 'Дополнительные сведения о: Visual C++ новых&#39;s с 2003 по 2015'
...
### <a name="conformance-improvements-in-visual-studio-2015"></a><a name="VS_RTM"></a> Улучшения соответствия в Visual Studio 2015
...
- **Обработка исключений**
   ...
   Чтобы обновить код, убедитесь, что конструктор копии для объекта исключения открыт и не помечен **`explicit`** .
...
### <a name="conformance-improvements-in-visual-studio-2015-update-2"></a><a name="VS_Update2"></a> Улучшения соответствия в обновлении 2 для Visual Studio 2015
...
### <a name="conformance-improvements-in-visual-studio-2015-update-3"></a><a name="VS_Update3"></a> Улучшения соответствия в обновлении 3 для Visual Studio 2015
...
**Переключить заголовок/файл кода.** Теперь можно переключаться между заголовком и соответствующим файлом кода с помощью команды контекстного меню или сочетания клавиш.
...
**Выделение ссылок.** Теперь при выборе символа выделяются все его экземпляры в текущем файле. Нажмите клавиши **CTRL** + **SHIFT** + **стрелка вверх** или **CTRL** + **SHIFT** + **стрелка вниз** для перемещения между выделенными ссылками. Вы можете отключить эту функцию в диалоговом окне **Параметры** в разделе **Текстовый редактор** > **C/C++** > **Дополнительно**.
...
С помощью **обозревателя архитектуры** можно исследовать ресурсы в решении C++, проектах или файлах. В строке меню выберите **архитектура**  >    >  **Обозреватель архитектуры** Windows. Вы можете выбрать нужный узел, например **Представление классов**. В этом случае правая часть окна инструментов развертывается, отображая список пространств имен. Если выбрать пространство имен, появляется новый столбец со списком классов, структур и перечислений в этом пространстве имен. Вы можете продолжить просмотр ресурсов или вернуться в крайний левый столбец для запуска другого запроса. См. раздел **Поиск кода с помощью обозревателя архитектуры**.
...
- В \<memory> заголовке новая функция make_shared является удобным, надежным и эффективным способом создания общего указателя на объект в то же время, когда создается объект.
- Заголовок поддерживает однонаправленные списки \<forward_list> .
...
- \<system_error>Заголовок и связанные шаблоны поддерживают обработку низкоуровневых системных ошибок. Члены класса `exception_ptr` можно использовать для переноса исключений между потоками.
- \<codecvt>Заголовок поддерживает преобразование различных кодировок символов Юникода в другие кодировки.
- \<allocators>Заголовок определяет несколько шаблонов, которые помогают выделять и освобождать блоки памяти для контейнеров на основе узлов.
- Существует множество обновлений \<random> заголовка.
...
**CTaskDialog.** Класс `CTaskDialog` можно использовать вместо стандартного окна сообщений `AfxMessageBox`. Класс `CTaskDialog` отображает и собирает больше данных, чем стандартное окно сообщений.
...
**Усовершенствования обработки ошибок IntelliSense.** Интегрированная среда разработки лучше распознает ошибки, которые могут привести к потере IntelliSense, и подчеркивает их красной волнистой линией. Кроме того, интегрированная среда разработки сообщает об ошибках IntelliSense в **окне Список ошибок**. Для отображения кода, который вызвал проблему, дважды щелкните ошибку в **окне списка ошибок**.
...
**Перейдите к.** Диалоговое окно **Переход к** позволяет найти все символы и файлы в проекте, соответствующие указанной строке. Результаты уточняются сразу после ввода дополнительных символов в строке поиска. В поле Feedback **Results** (Отзывы о результатах) отображается количество найденных элементов и помогает решить, следует ли ограничивать поиск. Поля **Вид/область**, **Расположение** и **Предварительный просмотр** помогают различать элементы с одинаковыми именами. Кроме того, вы можете расширить эту функцию для поддержки других языков программирования.
...
**Мастер элементов управления ATL.** Мастер элементов управления ATL больше не заполняет поле `ProgID` автоматически. Если элемент управления ATL не имеет программного идентификатора `ProgID`, другие средства могут с ним не работать. Одним из примеров инструмента, требующего элементы управления `ProgID`, является диалоговое окно **Вставка элемента управления ActiveX**. Дополнительные сведения о диалоговом окне см. в разделе [Вставка элементов управления ActiveX](../windows/adding-editing-or-deleting-controls.md#insert-activex-controls).
...
- Мастер **проектов ATL** теперь имеет возможность регистрации компонентов для всех пользователей. Начиная с Visual Studio 2008 COM-компоненты и библиотеки типов, создаваемые мастером **проектов ATL**, регистрируются в узле HKEY_CURRENT_USER реестра, если только не выбрана **регистрация компонентов для всех пользователей**.
- Мастер **проектов ATL** больше не предоставляет возможность создания проектов ATL с атрибутами. Начиная с Visual Studio 2008, мастер **проектов ATL** не имеет возможности изменять состояние нового проекта с атрибутами. Все создаваемые мастером проекты ATL стали неопределенными.
...
- Функция `__cpuid` обновлена. Теперь функции `__cpuid` и `__cpuidex` поддерживают несколько новых возможностей из последних ревизий процессоров AMD и Intel. Встроенная функция `__cpuidex` собирает больше данных на новых процессорах.
...
- `/Wp64`Параметр компилятора и **`__w64`** ключевое слово являются устаревшими. `/Wp64`Параметр компилятора и **`__w64`** ключевое слово, которые обнаруживают проблемы переносимости в 64 бит, являются устаревшими и будут удалены в следующей версии компилятора. Вместо них рекомендуется использовать MSVC, ориентированный на 64-разрядную платформу.
...
- Добавлены теги `/clr:pure`, `/clr:safe` и `/clr:oldSyntax`. (Впоследствии отмечены как нерекомендуемые в Visual Studio 2015 и удалены в Visual Studio 2017.)
- Нерекомендуемые параметры компилятора: многие параметры компилятора в этом выпуске являются устаревшими. Дополнительные сведения см. в разделе **Нерекомендуемые параметры компилятора** .
...
- Добавлен параметр компилятора `/G` (оптимизация под процессор).
- Параметры компилятора `/G3`, `/G4`, `/G5`, `/G6`, `/G7` и `/GB` удалены. Теперь компилятор использует "смешанную модель", которая пытается создать оптимальный выходной файл для всех архитектур.
- `/Gf` был удален. Используйте вместо него `/GF` (исключение повторяющихся строк).
...
- Эвристика встроенных функций обновлена; Дополнительные сведения см. в статьях **`inline`** , **`__inline`** **`__forceinline`** и **inline_depth** .
...
- Параметры компилятора `/ML` и `/MLd` удалены. Visual C++ больше не поддерживает однопоточную библиотеку CRT со статическим связыванием.
...
- Добавлен параметр компилятора `/QIPF_C` (список ошибок для пошагового выполнения C ЦП).
...
- Добавлен параметр `appdomain__declspec`.
...
- **`dynamic_cast`** Оператор содержит критические изменения.
- Собственные перечисления теперь позволяют задать базовый тип.
- Добавлен параметр `jitintrinsicdeclspec`.
- Добавлен параметр `noaliasdeclspec`.
- Добавлен параметр `process__declspec`.
- **абстрактные**, **переопределяемые** и **запечатанные** классы допустимы для компиляций в машинный код.
- **`__restrict`** Добавлено ключевое слово.
- Добавлен параметр `restrictdeclspec`.
- **`__thiscall`** теперь является ключевым словом.
- **`__unaligned`** Теперь задокументировано ключевое слово.
- **`volatile`** (C++) обновил поведение в отношении оптимизации.
...
- pragma `make_public`
- Обновлен синтаксис pragma `managed`, `unmanaged` (теперь имеется `push` и `pop`)
...
- Добавлен параметр компоновщика `/CLRUNMANAGEDCODECHECK` (добавление атрибута SuppressUnmanagedCodeSecurityAttribute).
...
- Удален параметр `/WS` программы editbin. Параметр `/WS` использовался для изменения образов, ориентированных на Windows NT 4.0. Вместо `/WS` можно использовать команду IMAGECFG.exe -R имя файла. IMAGECFG.exe находится на компакт-диске Windows NT 4.0 в каталоге SUPPORT\DEBUG\I386\IMAGECFG.EXE.
- Добавлен параметр /WX[:NO] программы lib.

### <a name="new-nmake-features"></a>Новые функции NMAKE

- Добавлен `/ERRORREPORT`.
- Добавлен `/G`.
...
- Были добавлены безопасные версии нескольких функций. Эти функции лучше обрабатывают ошибки и более строго контролируют буферы, что помогает устранить наиболее распространенные бреши безопасности. Новые безопасные версии идентифицируются по суффиксу **_s** .
...
- Функции, зависящие от языкового стандарта, теперь имеют версии, которые принимают языковой стандарт в качестве параметра, а не используют текущий языковой стандарт. Эти новые функции имеют суффикс **_l** . Для работы с объектами языковых стандартов добавлено несколько новых функций. К новым функциям относятся `_get_current_locale`, `_create_locale` и `_free_locale`.
...
- Функция `atoi` теперь возвращает INT_MAX и задает для `errno` значение ERANGE при переполнении. В предыдущих версиях поведение при переполнении определено не было.
...
- `_set_se_translator` теперь требует использования параметра компилятора `/EHa`.
- `fpos_t` теперь находится **`__int64`** в разделе `/Za` (для кода C) и когда __STDC__ задан вручную (для кода C++). Он использовался как **`struct`** .
...
- `_recalloc` объединяет возможности `realloc` и `calloc`.
...
- Удален параметр компилятора `/noBool`. Теперь компилятор допускает **`bool`** Отображение только в качестве ключевого слова (а не идентификатора) в файле исходного кода C++.
- **`long long`** Теперь тип доступен в виде заметки о **`typedef`** **`__int64`** том, что в CRT еще не поддерживается **`long long`** .
