---
description: 'Дополнительные сведения о: C6518'
title: C6518
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
- C6518
helpviewer_keywords:
- C6518
ms.assetid: c391a75b-9f16-43a5-a7cd-c5a233002850
ms.openlocfilehash: 4b32486b73cb6e78cd811fe6da5da518010f2edc
ms.sourcegitcommit: d6af41e42699628c3e2e6063ec7b03931a49a098
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 12/11/2020
ms.locfileid: "97135529"
---
# <a name="c6518"></a>C6518

> предупреждение C6518: недопустимая аннотация: свойство "SAL_writableTo" не может быть указано как предусловие для буферов, которые недоступны для записи: " \_ param \_ (1)"

Данное предупреждение указывает на наличие конфликта между значением свойства `SAL_writableTo` и перезаписываемым свойством. Обычно это означает, что свойство, доступное для записи, не имеет доступа на запись к параметру с заметками.

## <a name="example"></a>Пример

В следующем коде создается данное предупреждение, поскольку заметка `_Out_` компилируется для включения свойства `SAL_writableTo`, которое не допускает доступа для записи:

```cpp
#include <sal.h>
void f(_Out_ const char* pc)
{
    //code that can't write to *pc ...
}
```

Чтобы исправить это предупреждение, используйте следующий код:

```cpp
#include <sal.h>
void f(_Out_  char* pc)
{
    pc = "Hello World";
    //code ...
}
```
