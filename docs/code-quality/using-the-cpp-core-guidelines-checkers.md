...
1. Выберите  > страницу свойств **анализ кода** свойств конфигурации.
...
Библиотека поддержки нормативных руководств (GSL) разработана, чтобы помочь вам следовать основным рекомендациям. GSL включает определения, позволяющие заменять подверженные ошибкам конструкции более безопасными альтернативами. Например, можно заменить `T*, length` пару параметров на `span<T>` тип. Проект GSL доступен на сайте GitHub по адресу [https://github.com/Microsoft/GSL](https://github.com/Microsoft/GSL) . Библиотека является открытым исходным кодом, поэтому вы можете просматривать источники, создавать комментарии или участвовать в программе. Кроме того, с помощью диспетчера пакетов [vcpkg](../build/vcpkg.md) можно скачать и установить библиотеку локально.
