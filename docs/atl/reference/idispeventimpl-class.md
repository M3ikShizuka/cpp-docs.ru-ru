---
description: 'Дополнительные сведения о: IDispEventImpl Class'
...
*плибид*<br/>
Указатель на библиотеку типов, определяющий интерфейс диспетчеризации, на который указывает *пдиид*. Если **&GUID_NULL**, Библиотека типов будет загружена из объекта, который выберет события.
...
|[IDispEventImpl:: Жеттипеинфокаунт](#gettypeinfocount)|Извлекает число интерфейсов сведений о типе.|
...
> [!NOTE]
> `IDispEventImpl` и `IDispEventSimpleImpl` предоставляют собственные реализации, `IUnknown::QueryInterface` позволяющие каждому `IDispEventImpl` и `IDispEventSimpleImpl` базовому классу действовать как ОТДЕЛЬНОе удостоверение com, при этом разрешая прямой доступ к членам класса в основном COM-объекте.
...
## <a name="idispeventimplgetfuncinfofromid"></a><a name="getfuncinfofromid"></a> IDispEventImpl:: ЖетфунЦинфофромид
...
## <a name="idispeventimplgettypeinfocount"></a><a name="gettypeinfocount"></a> IDispEventImpl:: Жеттипеинфокаунт
...
## <a name="idispeventimplgetuserdefinedtype"></a><a name="getuserdefinedtype"></a> IDispEventImpl:: Жетусердефинедтипе
...
## <a name="idispeventimpltihclass"></a><a name="tihclass"></a> IDispEventImpl:: тихкласс
