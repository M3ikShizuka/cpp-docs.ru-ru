---
description: 'Дополнительные сведения: _mm_insert_si64, _mm_inserti_si64'
...
ms.locfileid: "97345268"
---
# <a name="_mm_insert_si64-_mm_inserti_si64"></a>_mm_insert_si64, _mm_inserti_si64

**Блок, относящийся только к системам Microsoft**

Формирует `insertq` инструкцию для вставки битов из второго операнда в первый операнд.
...
128-битовое поле, более 64 низкие биты которого содержат исходные младшие биты в *64, с* заданным битовым полем, замененным младшими битами *source2*. Верхние 64 разрядов возвращаемого значения не определены.
...
При вызове _mm_insert_si64 длина поля содержится в битах 77:72 source2 и индексе в битах 69:64.
...
Чтобы определить аппаратную поддержку для `insertq` инструкции, вызовите `__cpuid` встроенную функцию с параметром `InfoType=0x80000001` и установите бит 6 из `CPUInfo[2] (ECX)` . Этот бит равен 1, если инструкция поддерживается, и 0 в противном случае. Если запустить код, использующий встроенное на оборудовании, которое не поддерживает эту `insertq` инструкцию, результаты будут непредсказуемыми.
