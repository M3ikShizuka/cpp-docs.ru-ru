---
description: 'Дополнительные сведения: __svm_skinit'
title: __svm_skinit
ms.date: 09/02/2019
f1_keywords:
- __svm_skinit
...
Параметр *block_address* задает физический адрес блока памяти 64 КБ, называемого *блоком безопасного загрузчика* (SLB). SLB содержит программу, называемую *безопасным загрузчиком*. Он устанавливает операционную среду для компьютера, а затем загружает ядро безопасности.
