---
description: 'Дополнительные сведения о: CStatusBar Class'
...
|[CStatusBar:: Креатикс](#createex)|Создает `CStatusBar` объект с дополнительными стилями для внедренного `CStatusBarCtrl` объекта.|
...
1. Создайте `CStatusBar` объект.

1. Вызовите функцию [CREATE](#create) (или [креатикс](#createex)), чтобы создать окно строки состояния и присоединить его к `CStatusBar` объекту.
...
Дополнительные сведения об использовании `CStatusBar` см. в статье [Реализация строки состояния в MFC](../../mfc/status-bar-implementation-in-mfc.md) и в [техническом примечании 31: панели элементов управления](../../mfc/tn031-control-bars.md).
...
## <a name="cstatusbarcommandtoindex"></a><a name="commandtoindex"></a> CStatusBar:: Коммандтоиндекс
...
## <a name="cstatusbarcreateex"></a><a name="createex"></a> CStatusBar:: Креатикс
...
Конструирует `CStatusBar` объект, при необходимости создает шрифт строки состояния по умолчанию и устанавливает характеристики шрифта в значения по умолчанию.

```
CStatusBar();
```

## <a name="cstatusbardrawitem"></a><a name="drawitem"></a> CStatusBar::D Равитем
...
`itemAction`Элемент `DRAWITEMSTRUCT` структуры определяет выполняемое действие рисования. Переопределите эту функцию-член, чтобы реализовать Рисование для объекта, рисуемого владельцем `CStatusBar` . Приложение должно восстановить все объекты интерфейса графических устройств (GDI), выбранные для контекста вывода, указанного в *лпдравитемструкт* перед завершением этой функции-члена.

## <a name="cstatusbargetitemid"></a><a name="getitemid"></a> CStatusBar:: Жетитемид
...
## <a name="cstatusbargetitemrect"></a><a name="getitemrect"></a> CStatusBar:: Жетитемрект
...
*лпрект*<br/>
Указывает на структуру [Rect](/windows/win32/api/windef/ns-windef-rect) или объект [крект](../../atl-mfc-shared/reference/crect-class.md) , который будет принимать координаты индикатора, указанного параметром *ниндекс*.

### <a name="remarks"></a>Комментарии

Координаты задаются в пикселях относительно левого верхнего угла строки состояния.

## <a name="cstatusbargetpaneinfo"></a><a name="getpaneinfo"></a> CStatusBar:: Жетпанеинфо
...
## <a name="cstatusbargetpanestyle"></a><a name="getpanestyle"></a> CStatusBar:: Жетпанестиле
...
## <a name="cstatusbargetpanetext"></a><a name="getpanetext"></a> CStatusBar:: Жетпанетекст
...
## <a name="cstatusbargetstatusbarctrl"></a><a name="getstatusbarctrl"></a> CStatusBar:: Жетстатусбарктрл
...
## <a name="cstatusbarsetindicators"></a><a name="setindicators"></a> CStatusBar:: Сетиндикаторс
...
## <a name="cstatusbarsetpaneinfo"></a><a name="setpaneinfo"></a> CStatusBar:: Сетпанеинфо
...
- SBPS_NOBORDERS трехмерной границы вокруг области.

- SBPS_POPOUT обратную границу, чтобы текст "выталкивает".

- SBPS_DISABLED не выводить текст.

- SBPS_STRETCH области Stretch для заполнения неиспользуемого пространства. Только одна панель на строку состояния может иметь этот стиль.

- SBPS_NORMAL без растяжения, границ или всплывающего окна.

## <a name="cstatusbarsetpanestyle"></a><a name="setpanestyle"></a> CStatusBar:: Сетпанестиле
...
## <a name="cstatusbarsetpanetext"></a><a name="setpanetext"></a> CStatusBar:: Сетпанетекст
