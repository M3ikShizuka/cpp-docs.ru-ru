---
description: 'Дополнительные сведения о: CDocument Class'
...
|[CDocument:: адаптер](#getadapter)|Возвращает указатель на объект, реализующий `IDocument` интерфейс.|
|[CDocument:: Жетдоктемплате](#getdoctemplate)|Возвращает указатель на шаблон документа, описывающий тип документа.|
|[CDocument:: файл](#getfile)|Возвращает указатель на нужный `CFile` объект.|
...
|[CDocument:: Иссеарчандорганизехандлер](#issearchandorganizehandler)|Сообщает `CDocument` , был ли создан экземпляр объекта для поиска & Организации обработчика.|
...
|Имя|Описание|
|----------|-----------------|
|[CDocument:: m_bGetThumbnailMode](#m_bgetthumbnailmode)|Указывает, что `CDocument` объект был создан dllhost для эскизов. Должен быть возвращен `CView::OnDraw` .|
|[CDocument:: m_bPreviewHandlerMode](#m_bpreviewhandlermode)|Указывает, что `CDocument` объект был создан с помощью prevhost для `Rich Preview` . Должен быть возвращен `CView::OnDraw` .|
...
Дополнительные сведения о см `CDocument` . в разделах [сериализация](../../mfc/serialization-in-mfc.md), [документирование и просмотр архитектуры](../../mfc/document-view-architecture.md), а также [Создание документов и представлений](../../mfc/document-view-creation.md).
...
## <a name="cdocumentaddview"></a><a name="addview"></a> CDocument:: Аддвиев
...
## <a name="cdocumentbeginreadchunks"></a><a name="beginreadchunks"></a> CDocument:: Бегинреадчункс
...
## <a name="cdocumentcancloseframe"></a><a name="cancloseframe"></a> CDocument:: Канклосефраме
...
## <a name="cdocumentclearchunklist"></a><a name="clearchunklist"></a> CDocument:: Клеарчунклист
...
## <a name="cdocumentclearpathname"></a><a name="clearpathname"></a> CDocument:: Клеарпаснаме
...
Очистка пути от `CDocument` объекта приводит к тому, что приложение запрашивает пользователя при следующем сохранении документа. При этом команда **сохранения** работает так же, как команда **Сохранить как** .

## <a name="cdocumentdeletecontents"></a><a name="deletecontents"></a> CDocument::D Елетеконтентс

Вызвано платформой для удаления данных документа без уничтожения `CDocument` самого объекта.
...
## <a name="cdocumentfindchunk"></a><a name="findchunk"></a> CDocument:: Финдчунк
...
## <a name="cdocumentgetadapter"></a><a name="getadapter"></a> CDocument:: адаптер
...
## <a name="cdocumentgetdoctemplate"></a><a name="getdoctemplate"></a> CDocument:: Жетдоктемплате
...
## <a name="cdocumentgetfile"></a><a name="getfile"></a> CDocument:: файл
...
## <a name="cdocumentgetnextview"></a><a name="getnextview"></a> CDocument:: Жетнекствиев
...
*rPosition*<br/>
Ссылка на значение координаты, возвращенное предыдущим вызовом функций- `GetNextView` членов или [жетфирствиевпоситион](#getfirstviewposition) . Это значение не должно быть равно NULL.
...
## <a name="cdocumentgetpathname"></a><a name="getpathname"></a> CDocument:: путь
...
## <a name="cdocumentgetthumbnail"></a><a name="getthumbnail"></a> CDocument:: thumbnail
...
## <a name="cdocumentgettitle"></a><a name="gettitle"></a> CDocument::/Title
...
## <a name="cdocumentinitializesearchcontent"></a><a name="initializesearchcontent"></a> CDocument:: Инитиализесеарчконтент
...
## <a name="cdocumentissearchandorganizehandler"></a><a name="issearchandorganizehandler"></a> CDocument:: Иссеарчандорганизехандлер

Сообщает, был ли этот экземпляр `CDocument` создан для обработчика поиска & Организации.
...
## <a name="cdocumentloaddocumentfromstream"></a><a name="loaddocumentfromstream"></a> CDocument:: Лоаддокументфромстреам
...
Указывает, что `CDocument` объект был создан с помощью prevhost для расширенной предварительной версии. Должен быть возвращен `CView::OnDraw` .
...
## <a name="cdocumentonbeforerichpreviewfontchanged"></a><a name="onbeforerichpreviewfontchanged"></a> CDocument:: Онбефореричпревиевфонтчанжед
...
## <a name="cdocumentonchangedviewlist"></a><a name="onchangedviewlist"></a> CDocument:: Ончанжедвиевлист
...
## <a name="cdocumentonclosedocument"></a><a name="onclosedocument"></a> CDocument:: Онклоседокумент
...
## <a name="cdocumentoncreatepreviewframe"></a><a name="oncreatepreviewframe"></a> CDocument:: Онкреатепревиевфраме
...
## <a name="cdocumentondocumentevent"></a><a name="ondocumentevent"></a> CDocument:: Ондокументевент
...
События документа могут повлиять на несколько классов. Этот метод отвечает за обработку событий документа, которые влияют на классы, отличные от [класса CDocument](../../mfc/reference/cdocument-class.md). В настоящее время единственным классом, который должен отвечать на события документа, является [класс кдатарековерихандлер](../../mfc/reference/cdatarecoveryhandler-class.md). `CDocument`Класс имеет другие переопределяемые методы, отвечающие за обработку воздействия на `CDocument` .
...
## <a name="cdocumentonfilesendmail"></a><a name="onfilesendmail"></a> CDocument:: Онфилесендмаил
...
Специальная реализация `OnFileSendMail` for [коледокумент](../../mfc/reference/coledocument-class.md) правильно обрабатывает составные файлы.
...
## <a name="cdocumentonloaddocumentfromstream"></a><a name="onloaddocumentfromstream"></a> CDocument:: Онлоаддокументфромстреам
...
## <a name="cdocumentonnewdocument"></a><a name="onnewdocument"></a> CDocument:: Онневдокумент
...
Если пользователь выбирает команду Открыть файл в приложении SDI, платформа использует эту функцию для повторной инициализации существующего `CDocument` объекта вместо создания нового. Если пользователь выбирает файл, Открытый в приложении MDI, платформа создает новый `CDocument` объект каждый раз, а затем вызывает эту функцию для ее инициализации. Код инициализации необходимо поместить в эту функцию вместо в конструкторе, чтобы команда открытия файла действовала в приложениях SDI.
...
## <a name="cdocumentonpreviewhandlerqueryfocus"></a><a name="onpreviewhandlerqueryfocus"></a> CDocument:: Онпревиевхандлеркуерифокус

Направляет обработчик просмотра для возврата HWND, полученного из вызова `GetFocus` функции.
...
*фвнд*<br/>
заполняет При возврате из этого метода содержит указатель на HWND, возвращенный из вызова `GetFocus` функции из основного потока обработчика просмотра.
...
## <a name="cdocumentonpreviewhandlertranslateaccelerator"></a><a name="onpreviewhandlertranslateaccelerator"></a> CDocument:: Онпревиевхандлертранслатеакцелератор
...
## <a name="cdocumentonrichpreviewbackcolorchanged"></a><a name="onrichpreviewbackcolorchanged"></a> CDocument:: Онричпревиевбаккколорчанжед
...
## <a name="cdocumentonrichpreviewfontchanged"></a><a name="onrichpreviewfontchanged"></a> CDocument:: Онричпревиевфонтчанжед
...
## <a name="cdocumentonrichpreviewsitechanged"></a><a name="onrichpreviewsitechanged"></a> CDocument:: Онричпревиевситечанжед
...
## <a name="cdocumentonrichpreviewtextcolorchanged"></a><a name="onrichpreviewtextcolorchanged"></a> CDocument:: Онричпревиевтекстколорчанжед
...
## <a name="cdocumentonsavedocument"></a><a name="onsavedocument"></a> CDocument:: Онсаведокумент
...
## <a name="cdocumentonunloadhandler"></a><a name="onunloadhandler"></a> CDocument:: Онунлоадхандлер
...
## <a name="cdocumentonupdatefilesendmail"></a><a name="onupdatefilesendmail"></a> CDocument:: Онупдатефилесендмаил
...
В противном случае функция удаляет команду ID_FILE_SEND_MAIL из меню, включая разделители выше или ниже пункта меню. MAPI включается, если в пути есть MAPI32.DLL и в разделе [mail] файла WIN.INI MAPI = 1. Большинство приложений помещают эту команду в меню файл.

`CDocument` поддерживает отправку документа по электронной почте, если имеется поддержка почты (MAPI). См. статьи, [посвященные MAPI](../../mfc/mapi.md) и [поддержку MAPI в MFC](../../mfc/mapi-support-in-mfc.md).

## <a name="cdocumentprecloseframe"></a><a name="precloseframe"></a> CDocument::P Реклосефраме
...
*пфраме*<br/>
Указатель на объект [CFrameWnd](../../mfc/reference/cframewnd-class.md) , содержащий связанный `CDocument` объект.

### <a name="remarks"></a>Комментарии

Его можно переопределить, чтобы обеспечить пользовательскую очистку, но не забудьте также вызвать базовый класс.

Значение по умолчанию для `PreCloseFrame` не выполняет никаких действий в `CDocument` . `CDocument`Классы, производные от [Коледокумент](../../mfc/reference/coledocument-class.md) и [CRichEditDoc](../../mfc/reference/cricheditdoc-class.md) , используют эту функцию-член.

## <a name="cdocumentreadnextchunkvalue"></a><a name="readnextchunkvalue"></a> CDocument:: Реаднекстчунквалуе
...
## <a name="cdocumentreleasefile"></a><a name="releasefile"></a> CDocument:: Релеасефиле
...
Если *баборт* имеет значение true, `ReleaseFile` вызывается `CFile::Abort` , а файл освобождается. `CFile::Abort` исключение не создается.

Если *баборт* имеет значение false, `ReleaseFile` вызывается метод `CFile::Close` и файл освобождается.

Переопределите эту функцию-член, чтобы требовать действия от пользователя до освобождения файла.

## <a name="cdocumentremovechunk"></a><a name="removechunk"></a> CDocument:: Ремовечунк
...
## <a name="cdocumentremoveview"></a><a name="removeview"></a> CDocument:: Ремовевиев
...
## <a name="cdocumentreportsaveloadexception"></a><a name="reportsaveloadexception"></a> CDocument:: Репортсавелоадексцептион
...
## <a name="cdocumentsavemodified"></a><a name="savemodified"></a> CDocument:: Савемодифиед
...
## <a name="cdocumentsetchunkvalue"></a><a name="setchunkvalue"></a> CDocument:: Сетчунквалуе
...
## <a name="cdocumentsetpathname"></a><a name="setpathname"></a> CDocument:: Сетпаснаме
...
## <a name="cdocumentsettitle"></a><a name="settitle"></a> CDocument:: Сеттитле
