---
description: 'Дополнительные сведения о: CPropertyPage Class'
...
|[CPropertyPage:: Жетпсп](#getpsp)|Извлекает структуру [Пропшитпаже](/windows/win32/api/prsht/ns-prsht-propsheetpagea_v2) Windows, связанную с `CPropertyPage` объектом.|
...
## <a name="cpropertypagecanceltoclose"></a><a name="canceltoclose"></a> CPropertyPage:: Канцелтоклосе
...
`CancelToClose`Функция-член ничего не делает на немодальной странице свойств, так как немодальная страница свойств по умолчанию не имеет кнопки отмены.
...
Вызовите, `Construct` если не был вызван один из других конструкторов классов. `Construct`Функция-член является гибкой, поскольку можно оставить Оператор Parameter пустым, а затем указать несколько параметров и конструкцию в любой точке кода.

При `Construct` работе с массивами необходимо использовать метод `Construct` для каждого элемента массива, чтобы членам данных были назначены правильные значения.
...
## <a name="cpropertypagegetpsp"></a><a name="getpsp"></a> CPropertyPage:: Жетпсп

Извлекает структуру [Пропшитпаже](/windows/win32/api/prsht/ns-prsht-propsheetpagea_v2) Windows, связанную с `CPropertyPage` объектом.
...
Дополнительные сведения об этой структуре, включая список ее членов, см `PROPSHEETPAGE` . в разделе Windows SDK.
...
Когда платформа вызывает эту функцию, изменения, внесенные на всех страницах свойств на странице свойств, принимаются, на странице свойств остается фокус и `OnApply` возвращается значение true (значение 1). Прежде чем `OnApply` может быть вызвана платформой, необходимо вызвать метод [SetModified](#setmodified) и задать для его параметра значение true. Это активирует кнопку Применить сейчас, как только пользователь вносит изменения на странице свойств.
...
Реализация вызовов по умолчанию `OnApply` `OnOK` .
...
## <a name="cpropertypageonkillactive"></a><a name="onkillactive"></a> CPropertyPage:: Онкиллактиве
...
## <a name="cpropertypageonok"></a><a name="onok"></a> CPropertyPage:: ОНОК
...
Реализация по умолчанию этой функции-члена помечает страницу как "чистую" для отражения того, что данные были обновлены в `OnKillActive` функции.
...
## <a name="cpropertypageonsetactive"></a><a name="onsetactive"></a> CPropertyPage:: Онсетактиве
...
## <a name="cpropertypageonwizardback"></a><a name="onwizardback"></a> CPropertyPage:: Онвизардбакк
...
## <a name="cpropertypageonwizardfinish"></a><a name="onwizardfinish"></a> CPropertyPage:: Онвизардфиниш
...
## <a name="cpropertypageonwizardnext"></a><a name="onwizardnext"></a> CPropertyPage:: Онвизарднекст
...
## <a name="cpropertypagequerysiblings"></a><a name="querysiblings"></a> CPropertyPage:: Куерисиблингс
