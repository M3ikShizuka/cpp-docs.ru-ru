---
description: 'Дополнительные сведения о: AFX сообщения'
title: Сообщения AFX
ms.date: 11/04/2016
f1_keywords:
- SB_LINELEFT
...
|AFX_WM_RESETTOOLBAR|`CMFCToolBar`Объект отправляет это сообщение, когда панель инструментов восстанавливается в исходное состояние, то есть загружается из ресурсов. Обработайте это сообщение, чтобы повторно вставить кнопки на панели инструментов, классы которых являются производными от `CMFCToolbarButton` . Дополнительные сведения см. в разделе `CMFCToolbarComboBoxButton`.|Идентификатор ресурса панели инструментов, состояние которой было восстановлено.|Не используется.|Ноль.|
