---
description: 'Дополнительные сведения о: поддержка CCmdUI Class'
...
ms.locfileid: "97132994"
---
# <a name="ccmdui-class"></a>Класс поддержка CCmdUI

Используется только внутри `ON_UPDATE_COMMAND_UI` обработчика в классе, `CCmdTarget` производном от.
...
|[Поддержка CCmdUI:: Сетрадио](#setradio)|Как и `SetCheck` функция члена, но работает с группами переключателей.|
...
|[Поддержка CCmdUI:: m_pMenu](#m_pmenu)|Указывает на меню, представленное `CCmdUI` объектом.|
|[Поддержка CCmdUI:: m_pOther](#m_pother)|Указывает на объект окна, который отправил уведомление.|
|[Поддержка CCmdUI:: m_pSubMenu](#m_psubmenu)|Указывает на вложенное меню, представленное `CCmdUI` объектом.|
...
Когда меню извлекается, платформа ищет и вызывает каждый обработчик ON_UPDATE_COMMAND_UI, каждый обработчик вызывает функции-члены, `CCmdUI` такие как `Enable` и `Check` , а платформа соответствующим образом отображает каждый пункт меню.

Элемент меню может быть заменен кнопкой панели управления или другим объектом пользовательского интерфейса команды без изменения кода в `ON_UPDATE_COMMAND_UI` обработчике.

В следующей таблице приведена сводка `CCmdUI` функций-членов для различных элементов пользовательского интерфейса команды.

|User-Interface элемент|Включить|сетчекк|сетрадио|SetText|
|--------------------------|------------|--------------|--------------|-------------|
...
Дополнительные сведения об использовании этого класса см. в разделе [обновление объектов User-Interface](../../mfc/how-to-update-user-interface-objects.md).
...
## <a name="ccmduicontinuerouting"></a><a name="continuerouting"></a> Поддержка CCmdUI:: Континуераутинг
...
## <a name="ccmduienable"></a><a name="enable"></a> Поддержка CCmdUI:: enable
...
## <a name="ccmduim_nid"></a><a name="m_nid"></a> Поддержка CCmdUI:: m_nID
...
## <a name="ccmduim_nindex"></a><a name="m_nindex"></a> Поддержка CCmdUI:: m_nIndex
...
## <a name="ccmduim_pmenu"></a><a name="m_pmenu"></a> Поддержка CCmdUI:: m_pMenu
...
## <a name="ccmduim_psubmenu"></a><a name="m_psubmenu"></a> Поддержка CCmdUI:: m_pSubMenu

Указатель ( `CMenu` типа) на вложенное меню, представленное `CCmdUI` объектом.
...
## <a name="ccmduim_pother"></a><a name="m_pother"></a> Поддержка CCmdUI:: m_pOther
...
Значение NULL, если элемент является меню или не является `CWnd` объектом.

## <a name="ccmduisetcheck"></a><a name="setcheck"></a> Поддержка CCmdUI:: Сетчекк
...
## <a name="ccmduisetradio"></a><a name="setradio"></a> Поддержка CCmdUI:: Сетрадио
...
Эта функция-член действует аналогично `SetCheck` , за исключением того, что она работает с элементами пользовательского интерфейса, работающими как часть группы переключателей. Извлечение других элементов в группе выполняется автоматически, если только сами элементы не поддерживают поведение группы.

## <a name="ccmduisettext"></a><a name="settext"></a> Поддержка CCmdUI:: SetText
