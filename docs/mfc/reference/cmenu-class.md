...
|[Кмену:: Attach](#attach)|Присоединяет к объекту маркер меню Windows `CMenu` .|
...
|[Кмену::D Елететемпмап](#deletetempmap)|Удаляет все временные `CMenu` объекты, созданные `FromHandle` функцией члена.|
...
|[Кмену:: FromHandle](#fromhandle)|Возвращает указатель на объект, `CMenu` заданный маркером меню Windows.|
...
|[Кмену:: Жетсафехмену](#getsafehmenu)|Возвращает `m_hMenu` объект, упакованный этим `CMenu` объектом.|
...
Возвращает указатель на объект, `CMenu` заданный в меню маркером Windows.
...
## <a name="cmenugetmenuitemcount"></a><a name="getmenuitemcount"></a> Кмену:: Жетменуитемкаунт
