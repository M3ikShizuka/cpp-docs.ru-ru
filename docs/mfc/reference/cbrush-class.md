---
description: 'Дополнительные сведения о: Кбруш Class'
title: Класс Кбруш
ms.date: 11/04/2016
f1_keywords:
- CBrush
...
|[Кбруш:: FromHandle](#fromhandle)|Возвращает указатель на `CBrush` объект при наличии маркера для `HBRUSH` объекта Windows.|
...
|Имя|Описание|
|----------|-----------------|
|[Кбруш:: operator ХБРУШ](#operator_hbrush)|Возвращает маркер Windows, присоединенный к `CBrush` объекту.|

## <a name="remarks"></a>Комментарии

Чтобы использовать `CBrush` объект, создайте `CBrush` объект и передайте его в любую `CDC` функцию-член, для которой требуется кисть.

Кисти могут быть сплошными, заштрихованными или узорными.

Дополнительные сведения о см `CBrush` . в разделе [графические объекты](../../mfc/graphic-objects.md).
...
## <a name="cbrushcbrush"></a><a name="cbrush"></a> Кбруш:: Кбруш
...
*пбитмап*<br/>
Указывает на `CBitmap` объект, указывающий точечный рисунок, который рисует кисть.
...
## <a name="cbrushcreatedibpatternbrush"></a><a name="createdibpatternbrush"></a> Кбруш:: Креатедибпаттернбруш
...
*нусаже*<br/>
Указывает, должны ли `bmiColors[]` поля структуры данных [битмапинфо](/windows/win32/api/wingdi/ns-wingdi-bitmapinfo) (часть "упакованного" DIB ") содержать явные значения RGB или индексы в текущей логической палитре. Параметр должен иметь одно из следующих значений:
...
*лппаккеддиб*<br/>
Указывает на упакованный DIB, состоящий из `BITMAPINFO` структуры и сразу за которой следует массив байтов, определяющих Пиксели точечного рисунка.
...
- В первой версии для получения маркера рисунка DIB вы вызываете `GlobalAlloc` функцию Windows для выделения блока глобальной памяти, а затем заполняете память упакованным DIB.
...
- [Креатедибпаттернбруш](/windows/win32/api/wingdi/nf-wingdi-createdibpatternbrush) (Эта функция предоставляется только для совместимости с приложениями, написанными для более ранних версий Windows, чем 3,0; используйте `CreateDIBPatternBrushPt` функцию.)
...
## <a name="cbrushcreatehatchbrush"></a><a name="createhatchbrush"></a> Кбруш:: Креатехатчбруш
...
## <a name="cbrushcreatepatternbrush"></a><a name="createpatternbrush"></a> Кбруш:: Креатепаттернбруш
...
## <a name="cbrushcreatesolidbrush"></a><a name="createsolidbrush"></a> Кбруш:: Креатесолидбруш
...
*крколор*<br/>
Структура [COLORREF](/windows/win32/gdi/colorref) , указывающая цвет кисти. Цвет определяет значение RGB и может быть создан с помощью макроса RGB в WINDOWS. H.
...
## <a name="cbrushcreatesyscolorbrush"></a><a name="createsyscolorbrush"></a> Кбруш:: Креатесисколорбруш
...
## <a name="cbrushfromhandle"></a><a name="fromhandle"></a> Кбруш:: FromHandle
...
Если `CBrush` объект еще не присоединен к обработчику, `CBrush` создается и прикрепляется временный объект. Этот временный `CBrush` объект действителен только до тех пор, пока приложение не пойдет в цикле событий в следующий раз. В настоящее время все временные графические объекты будут удалены. Иными словами, временный объект действителен только во время обработки одного сообщения окна.
...
## <a name="cbrushgetlogbrush"></a><a name="getlogbrush"></a> Кбруш:: Жетлогбруш

Вызовите эту функцию члена, чтобы получить `LOGBRUSH` структуру.
...
Например, вызовите метод, `GetLogBrush` чтобы сопоставить определенный цвет или шаблон точечного рисунка.

### <a name="example"></a>Пример

[!code-cpp[NVC_MFCDocView#27](../../mfc/codesnippet/cpp/cbrush-class_7.cpp)]

## <a name="cbrushoperator-hbrush"></a><a name="operator_hbrush"></a> Кбруш:: operator ХБРУШ

Этот оператор используется для получения подключенного маркера GDI Windows `CBrush` объекта.
...
В случае успеха — обработчик для объекта Windows GDI, представленного `CBrush` объектом; в противном случае — null.
