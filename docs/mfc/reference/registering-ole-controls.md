...
В приведенном выше примере показано, как `AfxOleRegisterControlClass` вызывается с флагом для inserted и флагом для модели подразделения ORed вместе для создания шестого параметра:
