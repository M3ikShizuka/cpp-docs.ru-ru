---
description: 'Дополнительные сведения: службы диагностики'
...
в файле реализации все вызовы **`new`** будут хранить имя файла и номер строки, где была выполнена операция выделения памяти. Функция [CMemoryState::D умпаллобжектссинце](cmemorystate-structure.md#dumpallobjectssince) отображает эти дополнительные сведения, позволяя выявлять утечки памяти. Дополнительные сведения о результатах диагностики см. также в классе [CDumpContext](../../mfc/reference/cdumpcontext-class.md) .

Кроме того, библиотека времени выполнения C также поддерживает ряд диагностических функций, которые можно использовать для отладки приложений. Дополнительные сведения см. в разделе [подпрограммы отладки](../../c-runtime-library/debug-routines.md) в справочнике по библиотеке Run-Time.
...
|[Проверка](#verify)|Аналогично ASSERT, но вычисляет выражение в окончательной версии библиотеки, а также в отладочной версии.|
...
|[афксчеккеррор](#afxcheckerror)|Глобальная переменная, используемая для проверки переданного значения SCODE на предмет того, представляет ли оно ошибку; если да, вызывается соответствующая ошибка.|
|[афксчеккмемори](#afxcheckmemory)|Проверяет целостность всей выделенной на текущий момент памяти.|
...
|[афксдумпстакк](#afxdumpstack)|Создает образ текущего стека. Эта функция всегда компонуется статически.|
|[AfxEnableMemoryLeakDump](#afxenablememoryleakdump)|Включает дамп утечки памяти.|
|[афксенаблемеморитраккинг](#afxenablememorytracking)|Включает и отключает отслеживание памяти.|
|[афксисмемориблокк](#afxismemoryblock)|Проверяет, правильно ли выделен блок памяти.|
|[афксисвалидаддресс](#afxisvalidaddress)|Проверяет, находится ли диапазон адресов памяти в рамках области памяти программы.|
...
## <a name="afxdebugbreak"></a><a name="afxdebugbreak"></a> Функция AfxDebugBreak

Вызовите эту функцию, чтобы вызвать останов (в месте вызова `AfxDebugBreak` ) при выполнении отладочной версии приложения MFC.
...
*classname*<br/>
Имя `CObject` класса, производного от.
...
Использование `ASSERT_KINDOF` макроса точно так же, как и написание кода:
...
В окончательной версии MFC ASSERT_VALID ничего не делает. В отладочной версии он проверяет указатель, проверяет значение NULL и вызывает собственные `AssertValid` функции члена объекта. Если какой-либо из этих тестов завершается неудачей, предупреждающее сообщение отображается так же, как и [Assert](#assert).
...
После вставки этой директивы препроцессор вставит DEBUG_NEW везде, где вы используете **`new`** , а MFC сделает все остальное. При компиляции окончательной версии программы DEBUG_NEW разрешается в простую **`new`** операцию, а сведения о имени файла и номере строки не формируются.

> [!NOTE]
> В предыдущих версиях MFC (4,1 и более ранних версий) требовалось разместить `#define` инструкцию после всех инструкций, которые вызывали IMPLEMENT_DYNCREATE или IMPLEMENT_SERIAL макросы. Это больше не требуется.
...
В Windows NT и всех версиях Windows `afxDump` выходные данные отправляются в окно Output-Debug Visual C++ при отладке приложения.

Эта переменная определена только в отладочной версии MFC. Дополнительные сведения о см `afxDump` . в разделе [Отладка приложений MFC](/visualstudio/debugger/mfc-debugging-techniques).
...
Проверяются все блоки памяти, выделенные в куче, включая те, которые выделены, **`new`** но не выделены прямыми вызовами для базовых распределительов памяти, таких как функция **malloc** или `GlobalAlloc` функция Windows. Если обнаружено повреждение какого-либо блока, в выходные данные отладчика будет выведено сообщение.

Если включить строку

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]

в программном модуле последующие вызовы `AfxCheckMemory` показывают имя файла и номер строки, в которой была выделена память.

> [!NOTE]
> Если модуль содержит одну или несколько реализаций сериализуемых классов, необходимо вставить `#define` строку после последнего вызова макроса IMPLEMENT_SERIAL.
...
- Файл IMAGEHLP.DLL должен находиться в пути. Если эта библиотека DLL отсутствует, функция отобразит сообщение об ошибке. Сведения о наборе функций, предоставляемых IMAGEHLP, см. в разделе [Библиотека справки по изображениям](/windows/win32/Debug/image-help-library) .
...
Дополнительные сведения о см `AfxEnableMemoryTracking` . в разделе [Отладка приложений MFC](/visualstudio/debugger/mfc-debugging-techniques).
...
*плрекуестнумбер*<br/>
Указывает на **`long`** целое число, которое будет заполнено порядковым номером выделения блока памяти, или нуль, если он не представляет текущий активный блок памяти.
...
*бобжект*<br/>
Значение TRUE, если выделение предназначено для `CObject` производного объекта; в противном случае — значение false.
...
*PFN*<br/>
Указывает на функцию итерации, вызываемую для каждого класса. Аргументы функции являются указателем на `CRuntimeClass` объект и указателем void на дополнительные данные, которые вызывающий объект передает в функцию.
...
*PFN*<br/>
Указывает на функцию итерации для выполнения для каждого объекта. Аргументы функции являются указателем на `CObject` и пустым указателем на дополнительные данные, которые вызывающий объект передает в функцию.
