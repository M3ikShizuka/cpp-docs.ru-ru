---
description: 'Дополнительные сведения: конструктор лент (MFC)'
...
- Добавляет запись в скрипт определения ресурсов проекта (*.rc). В следующем примере IDR_RIBBON — уникальное имя, идентифицирующее ресурс ленты, RT_RIBBON_XML является типом ресурса, а RIBBON. mfcribbon-ms — это имя файла ресурсов.
...
- Создайте приложение MFC и настройте мастер проектов MFC для создания ленты. Дополнительные сведения см. [в разделе Пошаговое руководство. Создание ленточного приложения с помощью MFC](../mfc/walkthrough-creating-a-ribbon-application-by-using-mfc.md).
...
Если в проекте уже есть вручную закодированная лента, MFC предоставляет функции, которые можно использовать для преобразования существующей ленты в ресурс ленты. Дополнительные сведения см. в разделе [инструкции. преобразование существующей ленты MFC в ресурс ленты](../mfc/how-to-convert-an-existing-mfc-ribbon-to-a-ribbon-resource.md).
...
- **Область элементов:** Содержит элементы управления, которые можно перетаскивать в область конструктора.
...
- [Как настроить кнопку приложения](../mfc/how-to-customize-the-application-button.md)

- [Как настроить панель быстрого доступа](../mfc/how-to-customize-the-quick-access-toolbar.md)

- [Руководство. Добавление элементов управления ленты и обработчиков событий](../mfc/how-to-add-ribbon-controls-and-event-handlers.md)

- [Как загрузить ресурс ленты из приложения MFC](../mfc/how-to-load-a-ribbon-resource-from-an-mfc-application.md)
...
- **Панель:** Область ленты, на которой отображается группа связанных элементов управления. Каждая категория ленты содержит одну или несколько панелей ленты.

- **Элементы ленты:** Элементы управления на панелях, например кнопки и поля со списком. Сведения о различных элементах управления, которые можно разместить на ленте, см. в разделе [Пример риббонгаджетс: мини-приложения ленты](../overview/visual-cpp-samples.md).
