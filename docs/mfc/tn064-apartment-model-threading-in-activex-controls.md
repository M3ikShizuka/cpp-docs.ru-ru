---
description: 'Дополнительные сведения о: TN064: Apartment-Model потоков в элементах управления ActiveX'
...
В этом техническом примечании объясняется, как включить потоковую модель потоков в элементе управления ActiveX. Обратите внимание, что потоковые модели потоков поддерживаются только в Visual C++ версии 4,2 или более поздней.

## <a name="what-is-apartment-model-threading"></a>Что такое Apartment-Model многопоточность
...
## <a name="why-support-apartment-model-threading"></a>Зачем Apartment-Model поддержка потоков
...
Если элемент управления использует общие данные, например статическую переменную-член, доступ к этим данным должен быть защищен с помощью критической секции, чтобы предотвратить одновременное изменение данных несколькими потоками. Чтобы настроить критическую секцию для этой цели, объявите в классе элемента управления статическую переменную-член класса `CCriticalSection` . Используйте `Lock` `Unlock` функции члена и этого объекта критического раздела везде, где ваш код обращается к общим данным.
...
Элементы управления, поддерживающие потоковую модель потоков, должны указать эту возможность в реестре, добавив именованное значение «ThreadingModel» со значением «апартамент» в свою запись реестра ID класса в разделе *Class ID* \\ **InprocServer32** Key. Чтобы сделать этот ключ автоматически зарегистрированным для вашего элемента управления, передайте флаг *afxRegApartmentThreading* в шестом параметре в `AfxOleRegisterControlClass` :
...
Если проект элемента управления был создан с помощью Контролвизард в Visual C++ версии 4,1 или более поздней, этот флаг уже будет присутствовать в коде. Для регистрации потоковой модели изменения не требуются.
