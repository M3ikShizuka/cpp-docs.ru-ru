---
description: 'Дополнительные сведения: версии библиотеки MFC'
...
В проектах MFC символ препроцессора \_ Unicode используется для обозначения поддержки Юникода в кодировке UTF-16, а \_ MBCS — для обозначения поддержки MBCS. Эти параметры являются взаимоисключающими в проекте.
...
|Библиотека|Описание|
|-------------|-----------------|
|NAFXCW.LIB|Библиотека Static-Link MFC, версия выпуска|
|NAFXCWD.LIB|Библиотека Static-Link MFC, отладочная версия|
|УАФКСКВ. LIB|Библиотека MFC Static-Link с поддержкой Юникода, версия выпуска|
|УАФКСКВД. LIB|Библиотека MFC Static-Link с поддержкой Юникода, отладочная версия|
|АФКСНМКД. LIB|Библиотека MFC Static-Link без элементов управления диалогового окна MFC, версия выпуска|
|АФКСНМКДД. LIB|Библиотека MFC Static-Link без элементов управления диалогового окна MFC, отладочная версия|
...
|*Версия* MFCU.DLL|Библиотека DLL MFC, версия выпуска Юникод|
|*Версия* MFCD.DLL|DLL MFC, отладочная версия ANSI или MBCS|
|*Версия* MFCUD.DLL|Библиотека DLL MFC, отладочная версия Юникода|
|*Версия* мфкм. КОМПОНОВКИ|Библиотека DLL MFC с элементами управления Windows Forms, версией ANSI или MBCS|
|U.DLL *версии* мфкм|Библиотека DLL MFC с элементами управления Windows Forms, версия выпуска Юникод|
|D.DLL *версии* мфкм|Библиотека DLL MFC с элементами управления Windows Forms, отладочной версией ANSI или MBCS|
|UD.DLL *версии* мфкм|Библиотека DLL MFC с элементами управления Windows Forms, отладочная версия Юникода|
...
Если вы динамически связываетесь с общей библиотекой DLL MFC, будь это из приложения или библиотеки DLL расширения MFC, необходимо включить соответствующую *версию* MFC.U.DLL *версии* DLL или MFC при развертывании продукта.

Список Visual C++ библиотек DLL, которые могут распространяться вместе с приложениями, см. в разделе [распространяемый код для Microsoft Visual Studio 2017 и Microsoft Visual Studio 2017 SDK (включая служебные программы и файлы BuildServer)](/visualstudio/productinfo/2017-redistribution-vs) или [распространяемый код для Visual Studio 2019](/visualstudio/releases/2019/redistribution).
