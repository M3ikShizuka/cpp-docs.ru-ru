---
description: Дополнительные сведения см. в статье рекомендации по выбору класса коллекции.
...
### <a name="collection-shape-features"></a><a name="_core_collection_shape_features"></a> Функции фигур коллекции

|Фигурная|Заказано|Индексированного|Вставка элемента|Поиск указанного элемента|Дублирующиеся элементы|
|-----------|--------------|--------------|-----------------------|----------------------------------|-------------------------|
|Список|Да|Нет|Быстр.|Медл.|Да|
|Array|Да|По целому числу|Медл.|Медл.|Да|
...
### <a name="characteristics-of-mfc-collection-classes"></a><a name="_core_characteristics_of_mfc_collection_classes"></a> Характеристики классов коллекций MFC
...
1. Для сериализации необходимо явным образом вызвать функцию `Serialize` объекта коллекции. Для помещения в дамп необходимо явным образом вызвать его функцию `Dump`. Нельзя использовать форму `ar << collObj` для сериализации или формы `dmp` `<< collObj` для создания дампа.
...
[Руководство. Создание коллекции Type-Safe](../mfc/how-to-make-a-type-safe-collection.md)<br/>
[Доступ ко всем элементам коллекции](../mfc/accessing-all-members-of-a-collection.md)
