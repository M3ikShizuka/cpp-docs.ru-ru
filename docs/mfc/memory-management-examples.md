---
description: 'Дополнительные сведения см. в статье Управление памятью: примеры'
...
1. Используйте **`new`** оператор с синтаксисом массива, показанным в этом примере:
...
1. Используйте **`delete`** оператор следующим образом:
...
1. Используйте **`new`** для выделения структур данных в куче и **`delete`** их освобождения, как показано в следующих примерах:
...
1. Используйте **`new`** оператор, который возвращает указатель на объект, чтобы выделить объекты в куче. Используйте **`delete`** оператор, чтобы удалить их.
