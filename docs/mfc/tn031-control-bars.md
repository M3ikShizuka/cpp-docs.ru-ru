---
description: 'Дополнительные сведения о: TN031: панели элементов управления'
...
В этом заметке описываются классы управляющих панелей в MFC: общие [CControlBar](#_mfcnotes_ccontrolbar), [CStatusBar](#_mfcnotes_cstatusbar), [CToolBar](#_mfcnotes_ctoolbar), [CDialogBar](#_mfcnotes_cdialogbar)и `CDockBar` .

## <a name="ccontrolbar"></a><a name="_mfcnotes_ccontrolbar"></a> CControlBar

`ControlBar`— Это `CWnd` производный класс, который:
...
- CBRS_TOP (по умолчанию) закрепите панель элементов управления в верхней части страницы.

- CBRS_BOTTOM закрепить панель элементов управления внизу.

- CBRS_NOALIGN не изменять расположение панели управления при изменении размера родительского элемента.
...
- `CDockBar` Обобщенная область стыковки для других `CControlBar` производных объектов. Определенные функции-члены и переменные, доступные в этом классе, могут измениться в будущих версиях.
...
> [!NOTE]
> При создании собственного `CControlBar` производного класса вместо использования одного из производных классов MFC, таких как `CStatusBar` , `CToolBar` или `CDialogBar` , необходимо задать элемент данных *m_dwStyle* . Это можно сделать при переопределении `Create` :
...
Сообщения частного Windows в MFC, включая WM_SIZEPARENT, описаны в [техническом примечании 24](../mfc/tn024-mfc-defined-messages-and-resources.md).
...
Как правило, индикаторы обновляются с помощью механизма ON_UPDATE_COMMAND_UI. Во время простоя строка состояния будет вызывать обработчик ON_UPDATE_COMMAND_UI с ИДЕНТИФИКАТОРом строки на панели индикаторов.
...
Способ обновления кнопок панели инструментов всегда осуществляется с помощью механизма ON_UPDATE_COMMAND_UI. Во время простоя панель инструментов будет вызывать обработчик ON_UPDATE_COMMAND_UI с ИДЕНТИФИКАТОРом команды этой кнопки. ON_UPDATE_COMMAND_UI не вызывается для разделителей, но вызывается для кнопок и флажков.

Обработчик ON_UPDATE_COMMAND_UI может вызвать:

- `Enable`— Для включения или отключения кнопки. Она действует одинаково для кнопок и флажков.
...
API-интерфейсы настройки позволяют изменять состояние данной кнопки панели инструментов, желательно изменить эти состояния в обработчике ON_UPDATE_COMMAND_UI для команды, которую представляет кнопка панели инструментов. Помните, что обработка в режиме простоя изменит состояние кнопок панели инструментов с помощью обработчика ON_UPDATE_COMMAND_UI, поэтому любые изменения в этих состояниях, сделанные через Сетбуттонстиле, могут потеряться после следующего простоя.
...
- Нажатие кнопки мыши = TBBS_PRESSED (&#124; любой другой стиль)

- Кнопка недоступна — TBBS_DISABLED

- Кнопка нажата — TBBS_CHECKED

- Отключено = TBBS_CHECKED &#124; TBBS_DISABLED
...
Кнопки панели диалогового окна следует обновлять с помощью механизма обработчика ON_UPDATE_COMMAND_UI. Во время простоя диалоговая панель будет вызывать обработчик ON_UPDATE_COMMAND_UI с ИДЕНТИФИКАТОРом команды для всех кнопок с ИДЕНТИФИКАТОРом >= 0x8000 (то есть в диапазоне идентификаторов команд).
