---
description: 'Дополнительные сведения о: TN054: вызов DAO напрямую при использовании классов MFC DAO'
title: TN054. Вызов DAO напрямую при использовании классов DAO MFC
ms.date: 09/17/2019
helpviewer_keywords:
- MFC, DAO and
...
Для многих методов в интерфейсах DAO необходимо манипулировать `BSTR` объектом (строкой с префиксом длины, используемой в OLE-автоматизации). `BSTR`Объект обычно инкапсулируется в тип данных **Variant** . Сам класс MFC `COleVariant` наследуется от типа данных **Variant** . В зависимости от того, строится ли проект для ANSI или Unicode, интерфейсы DAO будут возвращать ANSI или Unicode `BSTR` . Два макроса, V_BSTR и V_BSTRT, полезны для гарантии того, что интерфейс DAO получает `BSTR` тип ожидаемого типа.

V_BSTR извлечет элемент *бстрвал* объекта `COleVariant` . Этот макрос обычно используется, когда необходимо передать содержимое в `COleVariant` метод интерфейса DAO. В следующем фрагменте кода показаны объявления и фактическое использование двух методов интерфейса DAO Дааусер, которые используют преимущества макроса V_BSTR:
...
Обратите внимание, что `VT_BSTRT` аргумент, указанный в приведенном `COleVariant` выше конструкторе, гарантирует, что `BSTR` в `COleVariant` случае сборки версии приложения в кодировке ANSI и Юникод `BSTR` для версии приложения в Юникоде будет использоваться символ ANSI. Именно именно это делает DAO.

Другой макрос, V_BSTRT, будет извлекать элемент *БСТРВАЛ* ANSI или Unicode в зависимости от `COleVariant` типа сборки (ANSI или Unicode). В следующем коде показано, как извлечь `BSTR` значение из в `COleVariant` `CString` :
...
Макрос V_BSTRT вместе с другими методами открытия других типов, хранящихся в `COleVariant` , демонстрируется в примере даовиев. В частности, этот перевод выполняется в `CCrack::strVARIANT` методе. По возможности этот метод преобразует значение в `COleVariant` экземпляр `CString` .
