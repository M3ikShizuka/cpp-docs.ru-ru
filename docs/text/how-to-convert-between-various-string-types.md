---
description: Дополнительные сведения о том, как выполнять преобразование между различными строковыми типами.
...
В этом примере показано, как преобразовать из в в `char *` другие типы строк, перечисленные выше. `char *`Строка (также известная как строка в стиле C) использует символ NULL для указания конца строки. Строки в стиле C обычно занимают один байт на символ, но также могут использовать два байта. В приведенных ниже примерах `char *` строки иногда называют строками многобайтовых символов из-за строковых данных, полученных в результате преобразования из строк Юникода. Функции однобайтовых и многобайтовых символов ( `MBCS` ) могут работать со `char *` строками.
...
В этом примере показано, как преобразовать из в в `wchar_t *` другие типы строк, перечисленные выше. Несколько строковых типов, включая `wchar_t *` , реализуют форматы расширенных символов. Для преобразования строки между многобайтовой кодировкой и расширенным символом можно использовать один вызов функции, например `mbstowcs_s` или вызов конструктора для класса, например `CStringA` .
...
В этом примере показано, как преобразовать из в в `_bstr_t` другие типы строк, перечисленные выше. `_bstr_t`Объект — это способ инкапсуляции строк расширенных символов `BSTR` . Строка BSTR имеет значение длины и не использует символ NULL для завершения строки, но тип строк, в который выполняется преобразование, может потребовать завершающего null.
...
В этом примере показано, как преобразовать из в в `CComBSTR` другие типы строк, перечисленные выше. Как и _bstr_t, `CComBSTR` объект — это способ инкапсуляции строк BSTR расширенных символов. Строка BSTR имеет значение длины и не использует символ NULL для завершения строки, но тип строк, в который выполняется преобразование, может потребовать завершающего null.
...
В этом примере показано, как преобразовать из в в `basic_string` другие типы строк, перечисленные выше.
...
В этом примере демонстрируется преобразование из строки расширенных символов (Юникод [) в](/dotnet/api/system.string) другие типы строк, перечисленные выше.
...
[Как преобразовать стандартную строку в System:: String](../dotnet/how-to-convert-standard-string-to-system-string.md)<br/>
[Как преобразовать System.: String в стандартную строку](../dotnet/how-to-convert-system-string-to-standard-string.md)<br/>
[Как преобразовать строку System:: String в wchar_t * или char\*](../dotnet/how-to-convert-system-string-to-wchar-t-star-or-char-star.md)<br/>
