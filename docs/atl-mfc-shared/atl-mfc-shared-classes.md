---
description: 'Дополнительные сведения: общие классы ATL и MFC'
title: Общие классы ATL-MFC
ms.custom: index-page
ms.date: 11/04/2016
ms.topic: conceptual
f1_keywords:
- vc.classes.shared
helpviewer_keywords:
- CPoint class, shared class
- CFileTimeSpan class, shared class
- COleDateTime class, shared class
- CFixedStringT class, shared class
- CStrBufT class, shared class
- CFileTime class, shared class
- CRect class, shared class
- CSimpleStringT class, shared class
- CStringT class, shared class
- CSize class, shared class
- CStringData class, shared class
- IAtlStringMgr class, shared class
- shared classes, MFC and ATL
- COleDateTimeSpan class, shared class
- CString objects, shared class
- shared classes
ms.assetid: e13aaac3-21ec-4f4d-8834-432b40fde544
ms.openlocfilehash: ba5927436b5f5c7bd31b21ec6dd7f187e2eb4e83
ms.sourcegitcommit: d6af41e42699628c3e2e6063ec7b03931a49a098
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 12/11/2020
ms.locfileid: "97142589"
---
# <a name="atlmfc-shared-classes"></a>Общие классы ATL и MFC

Эти вспомогательные классы можно использовать в любом собственном проекте C++, не требуя наличия библиотеки DLL MFC.

## <a name="in-this-section"></a>в этом разделе

[Классы, совместно используемые MFC и ATL](../atl-mfc-shared/reference/classes-shared-by-mfc-and-atl.md)<br/>
Предоставляет ссылки на классы, совместно используемые MFC и ATL.

## <a name="related-sections"></a>Связанные разделы

[Компоненты ATL COM Desktop](../atl/atl-com-desktop-components.md)<br/>
Справочные материалы по библиотеке ATL, которая представляет собой набор основанных на шаблонах классов C++, упрощающих написание объектов COM.

[Справочник по библиотека Microsoft Foundation Class (MFC)](../mfc/mfc-desktop-applications.md)<br/>
Содержит справочные материалы по библиотеке MFC — набору классов в, который представляет собой платформу приложения, написанную для Windows API.

[Отладка](/visualstudio/debugger/debugging-in-visual-studio)<br/>
Ссылки на разделы, описывающие использование отладчика Visual Studio для устранения логических ошибок в приложениях и хранимых процедурах.
