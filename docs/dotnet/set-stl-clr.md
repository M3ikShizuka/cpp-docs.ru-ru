---
description: 'Дополнительные сведения: Set (STL/CLR)'
title: set (STL/CLR)
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
...
|[operator! = (набор) (STL/CLR)](#op_neq)|Определяет `set` , не равен ли объект другому `set` объекту.|
|[Оператор< (Set) (STL/CLR)](#op_lt)|Определяет `set` , меньше ли объект, чем другой `set` объект.|
|[Оператор<= (Set) (STL/CLR)](#op_lteq)|Определяет, `set` является ли объект меньшим или равным другому `set` объекту.|
|[operator== (set) (STL/CLR)](#op_eq)|Определяет `set` , равен ли объект другому `set` объекту.|
|[Оператор> (Set) (STL/CLR)](#op_gt)|Определяет `set` , больше ли объект, чем другой `set` объект.|
|[Оператор>= (Set) (STL/CLR)](#op_gteq)|Определяет `set` , больше или равен ли объект другому `set` объекту.|
