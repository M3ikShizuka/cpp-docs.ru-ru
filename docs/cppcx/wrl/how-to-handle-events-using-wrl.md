---
description: Дополнительные сведения см. в статье как управлять событиями с помощью WRL.
...
ms.locfileid: "97229188"
---
# <a name="how-to-handle-events-using-wrl"></a>Практическое руководство. Обработка событий с использованием WRL

В этом документе показано, как использовать библиотеку шаблонов среда выполнения Windows C++ (WRL) для подписки на события объекта среда выполнения Windows и управления им.
...
5. Создайте фабрику активации для интерфейса `IDeviceWatcher`.
   ...
   Среда выполнения Windows использует полные имена для указания типов. `RuntimeClass_Windows_Devices_Enumeration_DeviceInformation`Параметр — это строка, предоставляемая среда выполнения Windows и содержащая имя требуемого класса среды выполнения.
