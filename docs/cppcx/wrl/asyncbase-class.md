---
description: 'Дополнительные сведения о: метод asyncbase Class'
...
`Start()` — Это защищенный метод, который не является видимым извне, так как асинхронные операции "горячий запуск" перед возвратом вызывающему объекту.
