---
description: 'Дополнительные сведения о: concurrent_vector классе'
...
|Имя|Описание|
|----------|-----------------|
|[станции\[\]](#operator_at)|Перегружен. Предоставляет доступ к элементу по заданному индексу в параллельном векторе. Этот метод является типобезопасным для операций чтения, а также при увеличении вектора, пока вы уверены, что значение `_Index` меньше, чем размер параллельного вектора.|
