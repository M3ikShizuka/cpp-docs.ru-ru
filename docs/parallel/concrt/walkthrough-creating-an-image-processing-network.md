---
description: Дополнительные сведения см. в разделе Пошаговое руководство. Создание сети Image-Processing
...
- [Пошаговое руководство. Создание агента потока данных](../../parallel/concrt/walkthrough-creating-a-dataflow-agent.md)
...
## <a name="defining-image-processing-functionality"></a><a name="functionality"></a> Определение функциональных возможностей обработки изображений
...
Следующая функция, `ProcessImage` , вызывает заданный объект [std:: Function](../../standard-library/function-class.md) для преобразования значения цвета каждого пикселя в [растровом](/windows/win32/api/gdiplusheaders/nl-gdiplusheaders-bitmap) объекте GDI+. `ProcessImage`Функция использует алгоритм [arallel_for concurrency::p](reference/concurrency-namespace-functions.md#parallel_for) для параллельной обработки каждой строки точечного рисунка.

[!code-cpp[concrt-image-processing-filter#3](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-image-processing-network_2.cpp)]

Следующие функции,, `Grayscale` , `Sepiatone` `ColorMask` и `Darken` вызывают `ProcessImage` функцию для преобразования значения цвета каждого пикселя в `Bitmap` объекте. Каждая из этих функций использует лямбда-выражение для определения преобразования цвета одного пикселя.

[!code-cpp[concrt-image-processing-filter#4](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-image-processing-network_3.cpp)]

Следующая функция, `GetColorDominance` , также вызывает `ProcessImage` функцию. Однако вместо изменения значения каждого цвета эта функция использует объекты [Concurrency:: combinable](../../parallel/concrt/reference/combinable-class.md) , чтобы вычислить, является ли изображение красным, зеленым или синим цветом частью изображения.
...
## <a name="creating-the-image-processing-network"></a><a name="network"></a> Создание сети обработки изображений
...
1. В `ProcessImages` функции создайте `countdown_event` переменную. `countdown_event`Класс показан далее в этом пошаговом руководстве.
...
1. Дождитесь, пока `countdown_event` переменная достигнет нуля.
...
|`save_bitmap`|`transformer`Объект, сохраняющий обработанные `image` на диск в виде точечного рисунка. `save_bitmap` Извлекает исходное имя файла из `map` объекта и изменяет расширение его имени на. bmp.|
...
`loaded_bitmaps`Буфер сообщений важен, так как в качестве `unbounded_buffer` объекта он предоставляет `Bitmap` объекты нескольким получателям. Если целевой блок принимает `Bitmap` объект, `unbounded_buffer` объект не предоставляет этот `Bitmap` объект другим целевым объектам. Поэтому важен порядок, в котором объекты связываются с `unbounded_buffer` объектом. `grayscale` `colormask` Блоки сообщений, и `sepiatone` используют фильтр, чтобы принимать только определенные `Bitmap` объекты. `decrement`Буфер сообщений является важной целью `loaded_bitmaps` буфера сообщений, так как он принимает все `Bitmap` объекты, отклоненные другими буферами сообщений. `unbounded_buffer`Объект необходим для распространения сообщений по порядку. Таким образом, `unbounded_buffer` объект блокируется до тех пор, пока новый целевой блок не будет связан с ним и не примет сообщение, если текущий целевой блок не принимает это сообщение.
...
`countdown_event`Объект в этом примере позволяет сети обработки изображений сообщать основному приложению о том, что все образы обработаны. `countdown_event`Класс использует объект [Concurrency:: Event](../../parallel/concrt/reference/event-class.md) , чтобы сообщить, что значение счетчика достигает нуля. Основное приложение увеличивает счетчик каждый раз при отправке имени файла в сеть. Узел терминала сети уменьшает счетчик после обработки каждого образа. Когда основное приложение проходит по указанному каталогу, оно ждет `countdown_event` , пока объект сообщит, что его счетчик достиг нуля.
...
Ниже приведен полный пример кода. `wmain`Функция управляет библиотекой GDI+ и вызывает `ProcessImages` функцию для обработки JPEG файлов в `Sample Pictures` каталоге.
...
![Пример выходных данных для данного примера](../../parallel/concrt/media/concrt_imageout.png "Пример выходных данных для данного примера")

`Lighthouse` создается объектом Tom Алфин, поэтому он преобразуется в градации серого. `Chrysanthemum`, `Desert` , `Koala` , и `Tulips` имеют красный цвет, поэтому компоненты синего и зеленого цветов удаляются и затемнены. `Hydrangeas`, `Jellyfish` и `Penguins` соответствуют критериям по умолчанию, поэтому выцветшей упрощенная.
