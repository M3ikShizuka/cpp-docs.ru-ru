---
description: Дополнительные сведения см. в разделе Пошаговое руководство. Отладка приложения C++ AMP
...
Инструкции по созданию проекта зависят от используемой версии Visual Studio. Убедитесь, что в оглавлении на этой странице выбрана правильная версия документации.
...
1. В отфильтрованном списке типов проектов щелкните **Консольное приложение**, а затем нажмите кнопку **Далее**. На следующей странице введите `AMPMapReduce` в поле **имя** , чтобы указать имя проекта, и укажите расположение проекта, если требуется другое.
...
1. В диалоговом окне **страницы свойств** в разделе **Свойства конфигурации** выберите предкомпилированные заголовки **C/C++**  >  .
...
4. Установите точки останова на строках кода, показанных на следующем рисунке (примерно в строках 67 строки 70).
   ...
   ![Точки останова ЦП отмечены рядом со строками кода в редакторе.](../../parallel/amp/media/campcpubreakpoints.png "Точки останова ЦП") <br/>
   Точки останова ЦП
...
4. Убедитесь, что в списке **Тип отладчика** выбрано значение **Авто** .
    ...
    Значение **Auto** является значением по умолчанию. В версиях до Windows 10 требуется **только значение GPU** , а не **Auto**.

5. Нажмите кнопку **ОК** .

6. Установите точку останова в строке 30, как показано на следующем рисунке.

   ![Точки останова GPU, отмеченные рядом со строкой кода в редакторе.](../../parallel/amp/media/campgpubreakpoints.png "Точки останова GPU") <br/>
   Точка останова GPU
...
2. Закрепите окно **потоков GPU** в нижней части Visual Studio. Нажмите кнопку **развернуть потоковый переключатель** , чтобы отобразить текстовые поля плитки и потока. В окне **потоков GPU** отображается общее число активных и заблокированных потоков GPU, как показано на следующем рисунке.
   ...
   ![Окно потоков GPU с 4 активными потоками.](../../parallel/amp/media/campc.png "Окно "Потоки GPU" с 4 активными потоками") <br/>
   Окно "Потоки GPU"

   313. выделено плиток для этого вычисления. Каждая плитка содержит 32 потоков. Поскольку локальная отладка GPU выполняется в эмуляторе программного обеспечения, существует четыре активных потока GPU. Четыре потока выполняют инструкции одновременно, а затем переходят к следующей инструкции.
   ...
   В окне **потоков GPU** имеется четыре активных потока GPU и 28 потоков GPU, заблокированных на [tile_barrier:: wait](reference/tile-barrier-class.md#wait) , определенный в около строки 21 ( `t_idx.barrier.wait();` ). Все потоки GPU 32 принадлежат к первой плитке, `tile[0]` . Стрелка указывает на строку, содержащую текущий поток. Чтобы переключиться на другой поток, используйте один из следующих методов.
    ...
    - В строке потока для переключения в окне **потоки GPU** откройте контекстное меню и выберите **Переключение к потоку**. Если строка представляет более одного потока, переключиться на первый поток в соответствии с координатами потока.
...
1. Чтобы открыть окно **Параллельные стеки** , в строке меню выберите **Отладка**  >    >  **параллельных стеков** Windows.
...
3. Убедитесь, что в списке в левом верхнем углу выбраны **потоки** . На следующем рисунке в окне **Параллельные стеки** показано представление потоков GPU, которые вы видели в окне **потоков GPU** , в стеке вызовов.
   ...
   ![Окно параллельных стеков с 4 активными потоками.](../../parallel/amp/media/campd.png "Окно "Параллельные стеки" с 4 активными потоками") <br/>
   Окно "Параллельные стеки"

   32. потоки перешли в `_kernel_stub` лямбда-инструкцию в `parallel_for_each` вызове функции, а затем на `sum_kernel_tiled` функцию, где происходит снижение параллелизма. 28 из 32 потоков продвигаемся к [`tile_barrier::wait`](reference/tile-barrier-class.md#wait) инструкции и остаются заблокированными в строке 22, а остальные четыре потока остаются активными в `sum_kernel_tiled` функции в строке 30.

   Можно проверить свойства потока GPU. Они доступны в окне **потоки GPU** в виде богатой подсказки в окне **Параллельные стеки** . Чтобы увидеть их, наведите указатель мыши на кадр стека `sum_kernel_tiled` . На следующем рисунке показана подсказка.

   ![Подсказка для окна параллельных стеков.](../../parallel/amp/media/campe.png "DataTip для окна "Параллельные стеки"") <br/>
...
1. Чтобы открыть окно **параллельных контрольных значений** , в строке меню выберите **Отладка** параллельных  >    >  **контрольных значений Windows параллельный** Просмотр  >  **1**.
...
5. Снова щелкните заголовок столбца **добавить контрольные значения** , введите `localA[localIdx[0]]` , а затем нажмите клавишу **Ввод** .
   ...
   ![Параллельное окно контрольных значений с отсортированными результатами.](../../parallel/amp/media/campf.png "Окно "Контроль параллельных данных" с отсортированными результатами") <br/>
   Результаты сортировки

   Вы можете экспортировать содержимое в окно " **контроль параллельных** данных" в Excel, нажав кнопку **Excel** и выбрав пункт **Открыть в Excel**. Если на компьютере разработчика установлен Excel, кнопка открывает лист Excel, содержащий содержимое.
...
6. В правом верхнем углу окна " **контроль параллельных** данных" есть элемент управления фильтра, который можно использовать для фильтрации содержимого с помощью логических выражений. Введите `localA[localIdx[0]] > 20000` в текстовом поле элемент управления фильтр, а затем нажмите клавишу **Ввод** .
   ...
   Теперь окно содержит только потоки, в которых `localA[localIdx[0]]` значение больше 20000. Содержимое по-прежнему сортируется по `localA[localIdx[0]]` столбцу, что является действием сортировки, выбранным ранее.
...
3. Выберите символ флага в левой части строки, содержащий четыре активных потока.
   ...
   ![Окно потоков GPU с помеченными потоками.](../../parallel/amp/media/campg.png "Окно "Потоки GPU" с помеченными потоками") <br/>
...
4. Если вы хотите сосредоточиться на четырех отмеченных потоках, можно выбрать отображение только отмеченных потоков. Он ограничивает то, что отображается в окнах **потоков GPU**, **параллельных контрольных значений** и **параллельных стеков** .
   ...
   ![Панель инструментов расположения отладки с значком "Показывать только помеченные".](../../parallel/amp/media/camph.png "Панель инструментов "Место отладки" со значком "Показать только помеченные"") <br/>
...
3. Откройте контекстное меню активной строки и выберите пункт **заморозить**.
   ...
   ![Окна потоков GPU, отображающие замороженные потоки.](../../parallel/amp/media/campk.png "Окно "Потоки GPU", в котором показаны замороженные потоки") <br/>
...
1. В контекстном меню для одного из потоков в окне **потоки GPU** выберите **Группировать по**, **адрес**.
   ...
   ![Окно потоков GPU с потоками, сгруппированными по адресу.](../../parallel/amp/media/campl.png "Окно "Потоки GPU" с потоками, сгруппированными по адресам") <br/>
   Сгруппированные потоки в окне **потоков GPU**

2. Можно также выполнить операцию **Group By** , открыв контекстное меню сетки данных окна **контрольных значений** . Выберите **Группировать по**, а затем выберите пункт меню, соответствующий тому, как нужно группировать потоки.
...
3. В контекстном меню **редактора кода** выберите пункт **запустить текущую плитку в курсоре**.
   ...
   24 потока, которые ранее были заблокированы в барьере в строке 21, были выполнены до строки 32. Он отображается в окне **потоков GPU** .

## <a name="see-also"></a>См. также раздел

[Обзор C++ AMP](../../parallel/amp/cpp-amp-overview.md)\
[Отладка кода GPU](/visualstudio/debugger/debugging-gpu-code)\
[Как использовать окно "потоки GPU"](/visualstudio/debugger/how-to-use-the-gpu-threads-window)\
[Как использовать параллельную окно контрольных значений](/visualstudio/debugger/how-to-use-the-parallel-watch-window)\
