---
description: 'Дополнительные сведения о: б. заглушки для функций библиотеки времени выполнения'
...
ms.locfileid: "97342521"
---
# <a name="b-stubs-for-run-time-library-functions"></a>Б. Заглушки для функций библиотеки среды выполнения

В этом разделе приводятся заглушки для функций библиотеки времени выполнения, определенных в интерфейсе API OpenMP C и C++. Заглушки предоставляются для обеспечения переносимости на платформы, которые не поддерживают API OpenMP C и C++. На этих платформах программы OpenMP должны быть связаны с библиотекой, содержащей эти функции-заглушки. Функции-заглушки предполагают, что директивы в программе OpenMP игнорируются. Таким образом, они имитируют последовательную семантику.

> [!NOTE]
> Переменная блокировки, которая отображается в функциях блокировки, должна быть доступна исключительно через эти функции. Его не следует инициализировать или иным образом изменить в пользовательской программе. Пользователи не должны делать предположения о механизмах, используемых реализациями OpenMP C и C++ для реализации блокировок на основе схемы, используемой функциями-заглушками.
